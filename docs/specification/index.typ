...
The #strong[User Services Platform]; is a standardized #strong[protocol
for managing, monitoring, upgrading, and controlling connected
devices];. USP allows service providers, consumer electronics
manufacturers, and end users to:

- Create interoperable and vendor\-independent #strong[managed Wi\-Fi
  systems];.
- Perform #strong[lifecycle management]; of consumer connected devices
  and containerized microservices.
- Support #strong[independent applications]; for application\-enabled
  services gateways.
- Enable #strong[IoT and consumer electronics upgradability]; for
  critical security patches.
- Develop applications that gather the telemetry necessary to
  #strong[mass data processing, AI, and machine learning];.
- #strong[Bootstrap and configure]; newly installed or purchased devices
  and applications.
- Let customer support #strong[monitor and troubleshoot]; connected
  devices, services, and home network links.
- Easily map the home network to #strong[control service quality and
  monitor threats];.
- Securely #strong[control IoT, smart home, and smart networking
  functions]; locally or from the Cloud.
- #strong[Enable multi\-tenant]; (i.e., multi\-stakeholder) management
  and control.
...
networks],
    ...
    titleid: [title],
    toc: [false],
    usp-msg-proto-file: [usp\-msg\-1\-4.proto],
    usp-msg-proto-url: [..\/specification\/usp\-msg\-1\-4.proto],
    usp-record-proto-file: [usp\-record\-1\-4.proto],
    usp-record-proto-url: [..\/specification\/usp\-record\-1\-4.proto],
    version: [],
    versionNumber: [1.4],
...
#bbf-issue-history[
  ...
  #table(
    ...
    [Regenerated data model HTML using fixed version of the BBF report
    tool
    ],
    [Release 1.1.2
    ],
    ...
    [#link("https://www.broadband-forum.org/download/TR-369_Amendment-4.pdf")[Release
    1.4]
    ...
    - Updated ResolvedPathResult Fields to clarify that R\-GET.4 does
      not return parameters for which the Controller does not have Read
      permission for.
    - Added Appendix VI Usage of the Register Operation to define the
      Rules related to USP Services Registering data model paths.
    - Added TLS Support to the UDS MTP impacting Section Handling
      Failures to Deliver USP Records with new requirements (R\-UDS.23a
      and R\-UDS.23b) and updates to Appendix V1 Basic Solution
      Concepts.
    - Expanded Error Codes Section Applicability to multiple error codes
      from Error Message to Any, to send an operate response rather than
      USP error.
    - Updated GetSupportedDM Request Fields Section to expand the
      Register Message for Commands, Events, and Parameters. Updated
      Definitions Section with new definitions for Command Path and
      Event Path.
    - Updated R\-OPR.4 and R\-OPR.5 to align with R\-OPR.1.
    - Updated Appendix 1 Basic Solution Concepts Section to define that
      the Execution Environment is no longer static, and is managed via
      USP Software Module Management.
    - Updated Handling of WebSocket Frames Section adding R\-WS.14b to
      discourage the use of WebSocket fragmentation.
    - Updated Handling of the WebSocket Session Section adding note to
      R\-WS.8 to clarify the encoding of USP Endpoint ID for URI use.
    - Opened up the OUI definition and Use of authorityscheme and
      authorityid Section R\-ARC.2a to allow for not only the old
      24\-bit OUIs but to also allow for 36\-bit OUIs.
    - Updated OnBoardRequest Section to state that the OnBoardRequest
      notification MUST be sent with send\_resp\=true, which included
      updates to R\-NOT.5 and R\-NOT\-6 as well as the addition of
      R\-NOT\-6a to insure the Controller retries until a notification
      response is recieved.
    - Defined the term "Partial Path" in Definitions Section.
    - Fixed the Operate example in Operate Examples Section to include
      both sync and async data model commands.
    - Updated the GetSupportedDM to now indicate the unique keys of a
      table, which caused updates to the examples in GetSupportedDM
      Examples Section and the usp\-msg\-1\-4.proto file within the
      GetSupportedDM message.
    - Updated Role Definition Section with clarifications to Role
...
More information can be found in RFC 2119 #link(<ref-RFC2119>)[\[9\]]
for key words defined there. Additional key words defined in the context
of this specification are DEPRECATED and OBSOLETED.
...
#emph[#link("https://www.broadband-forum.org/technical/download/TR-069.pdf")[CPE
WAN Management Protocol]];, Broadband Forum, 2018]
] <ref-TR-069>

#bbf-csl-entry[
#bbf-csl-left-margin[\[2\] ]#bbf-csl-right-inline[TR\-106,
#emph[#link("https://data-model-template.broadband-forum.org")[Data
Model Template for CWMP Endpoints and USP Agents]];, Broadband Forum]
] <ref-TR-106>

#bbf-csl-entry[
#bbf-csl-left-margin[\[3\] ]#bbf-csl-right-inline[TR\-181 Issue 2,
#emph[#link("https://usp-data-models.broadband-forum.org#Device:2")[Device
Data Model]];, Broadband Forum]
] <ref-TR-181>

#bbf-csl-entry[
#bbf-csl-left-margin[\[4\] ]#bbf-csl-right-inline[Protocol Buffers v3,
#emph[#link("https://developers.google.com/protocol-buffers/docs/proto3")[Protocol
Buffers Mechanism for Serializing Structured Data Version 3]];, Google]
] <ref-PROTOBUF>

#bbf-csl-entry[
#bbf-csl-left-margin[\[5\] ]#bbf-csl-right-inline[IMEI Database,
#emph[#link("https://imeidb.gsma.com/imei/index#")[International Mobile
Equipment Identity]];, GSMA]
] <ref-IMEI>

#bbf-csl-entry[
#bbf-csl-left-margin[\[6\] ]#bbf-csl-right-inline[IANA,
#emph[#link("https://www.iana.org/")[Internet Assigned Numbers
Authority]];, IANA]
] <ref-IANA>

#bbf-csl-entry[
#bbf-csl-left-margin[\[7\] ]#bbf-csl-right-inline[Assignments,
#emph[#link("https://regauth.standards.ieee.org")[IEEE Registration
Authority]];, IEEE]
] <ref-IEEEREG>

#bbf-csl-entry[
#bbf-csl-left-margin[\[8\] ]#bbf-csl-right-inline[RFC 1035,
#emph[#link("https://tools.ietf.org/html/rfc1035")[Domain Names \-
Implementation and Specification]];, IETF, 1987]
] <ref-RFC1035>

#bbf-csl-entry[
#bbf-csl-left-margin[\[9\] ]#bbf-csl-right-inline[RFC 2119,
#emph[#link("https://tools.ietf.org/html/rfc2119")[Key words for use in
RFCs to Indicate Requirement Levels]];, IETF, 1997]
] <ref-RFC2119>

#bbf-csl-entry[
#bbf-csl-left-margin[\[10\] ]#bbf-csl-right-inline[RFC 2141,
#emph[#link("https://tools.ietf.org/html/rfc2141")[URN Syntax]];, IETF,
1997]
] <ref-RFC2141>

#bbf-csl-entry[
#bbf-csl-left-margin[\[11\] ]#bbf-csl-right-inline[RFC 2234,
#emph[#link("https://tools.ietf.org/html/rfc2234")[Augmented BNF for
Syntax Specifications: ABNF]];, IETF, 1997]
] <ref-RFC2234>

#bbf-csl-entry[
#bbf-csl-left-margin[\[12\] ]#bbf-csl-right-inline[RFC 3279,
#emph[#link("https://datatracker.ietf.org/doc/html/rfc3279")[Algorithms
...
#bbf-csl-entry[
#bbf-csl-left-margin[\[13\] ]#bbf-csl-right-inline[RFC 3315,
#emph[#link("https://tools.ietf.org/html/rfc3315")[Dynamic Host
Configuration Protocol for IPv6 (DHCPv6)]];, IETF, 2003]
] <ref-RFC3315>

#bbf-csl-entry[
#bbf-csl-left-margin[\[14\] ]#bbf-csl-right-inline[RFC 3925,
#emph[#link("https://tools.ietf.org/html/rfc3925")[Vendor\-Identifying
Vendor Options for Dynamic Host Configuration Protocol version 4
(DHCPv4)]];, IETF, 2004]
] <ref-RFC3925>

#bbf-csl-entry[
#bbf-csl-left-margin[\[15\] ]#bbf-csl-right-inline[RFC 3986,
#emph[#link("https://tools.ietf.org/html/rfc3986")[Uniform Resource
...
the Context of Transport Layer Security (TLS)]];, IETF, 2011]
] <ref-RFC6125>
...
#emph[#link("https://stomp.github.io/stomp-specification-1.2.html")[Simple
Text Oriented Message Protocol]];]
] <ref-STOMP-1-2>
] <refs>]
...
A Command Path is a Path Name that addresses a Command of an Object or
Object Instance. See #link(<sec:path-names>)[Path Names].

#strong[Connection Capabilities];
...
A Event Path is a Path Name that addresses an Event of an Object or
Object Instance. See #link(<sec:path-names>)[Path Names].
...
#strong[Message Transfer Protocol];
...
Instance, Command, Event, or Parameter in an Agent’s Instantiated or
Supported Data Model. See #link(<sec:path-names>)[Path Names].
...
#[
  ...
  #table(
    ...
    [Get Supported Data Model (informal of GetSupportedDM message)
    ],
    [HMAC
    ],
...
#strong[Privacy]; is the right of an individual or group to control or
influence what information related to them may be collected, processed,
and stored and by whom, and to whom that information may be disclosed.

#strong[Assurance of privacy]; depends on whether stakeholders expect,
or are legally required, to have information protected or controlled
from certain uses. As with security, the ability for users to control
who has access to their data is of primary importance in the world of
the connected user, made clear by users as well as regulators.
...
#figure(
  ...
  #bbf-image("architecture/usp_architecture.png")<img:usp-agent-and-controller-architecture>
...
=== 2.2.1 Use of authority\-scheme and authority\-id <sec:use-of-authority-scheme-and-authority-id>
...
#strong[#[R\-ARC.0]<r-arc.0>]; \- A Controller and Agent within the same
USP Domain MAY use the same Endpoint ID.

#strong[#[R\-ARC.1]<r-arc.1>]; \- Endpoints MUST tolerate the same
Endpoint ID being used by an Agent and a Controller in the same USP
Domain.

#strong[#[R\-ARC.2]<r-arc.2>]; \- Endpoints that share the same Endpoint
ID MUST NOT communicate with each other via USP.
...
#strong[#[R\-ARC.2a]<r-arc.2a>]; \- Endpoints MUST follow the
...
#[
  ...
  #table(
    ...
    [usage and rules for authority\-id and instance\-id
    ]),
    [`oui`
    ],
    [`authority-id` MUST be an OUI (now called "MAC Address Block")
    assigned and registered by the IEEE Registration Authority
    #link(<ref-IEEEREG>)[\[7\]] to the entity responsible for this
    Endpoint. authority\-id MUST use hex encoding of the 24, 28, or
    36\-bit ID (resulting in 6, 7 or 9 hex characters). `instance-id`
    syntax is defined by this entity, who is also responsible for
    determining instance\-id assignment mechanisms and for ensuring
    uniqueness of the instance\-id within the context of the OUI.
    ...
    registered by IANA #link(<ref-IANA>)[\[6\]] to the entity
    responsible for this Endpoint. `authority-id` MUST use decimal
    encoding of the IANA\-assigned number. \
    `instance-id` syntax is defined by this entity, who is also
    responsible for determining instance\-id assignment mechanisms and
    for ensuring uniqueness of the instance\-id within the context of
    ...
    [When present, an `authority-id` for "`self`" MUST be between 1 and
    6 non\-reserved characters in length. When present, it is generated
    by the Endpoint. It is not required to have an `authority-id` for
    "`self`". \
    ...
    [An `authority-id` for "`user`" MUST be between 0 and 6
    non\-reserved characters in length. \
    ...
    `instance-id`is `<OUI> "-" <SerialNumber>`, with `OUI` as defined
    above. \
    ...
    [`authority-id` MUST be zero\-length. \
    `instance-id` is `<OUI> "-" <ProductClass> "-" <SerialNumber>`, with
    `OUI` as defined above. \
    ...
    [`authority-id` MUST be zero\-length. \
    `instance-id` is an IMEI #link(<ref-IMEI>)[\[5\]] as defined by
    GSMA. \
...
#strong[#[R\-ARC.3]<r-arc.3>]; \- BBF OUI (`00256D`) and Private
Enterprise Number (`3561`) are reserved for use in BBF documentation and
BBF prototyping and MUST NOT be used by any entity other than BBF.

#strong[#[R\-ARC.4]<r-arc.4>]; \- The "`proto`" and "`doc`"
...
#strong[#[R\-ARC.4a]<r-arc.4a>]; \- If the `authority-scheme` `fqdn` is
...
#strong[#[R\-ARC.5]<r-arc.5>]; \- `instance-id` MUST be encoded using
...
notation of RFC 2234 #link(<ref-RFC2234>)[\[11\]], including the
...
from RFC 3986 #link(<ref-RFC3986>)[\[15\]] as the set of unreserved
characters and percent\-encoded characters that are acceptable for all
components of a URI. This set is also allowed for use in URNs RFC 2141
#link(<ref-RFC2141>)[\[10\]], and all MTP headers.

#strong[#[R\-ARC.6]<r-arc.6>]; \- An `instance-id` value MUST be no more
...
#emph[Note: This should be understood by those familiar with CWMP. For
...
Protocol (SNMP) or YANG definitions used in NETCONF.];
...
#strong[#[R\-ARC.7]<r-arc.7>]; \- All USP Endpoints MUST support the
Path Name syntax as defined in TR\-106 #link(<ref-TR-106>)[\[2\]].
...
+ Object Path \- This is a Path Name of either a Single\-Instance
  ...
  An Object Path, as with an Object Instance Path, is sometimes referred
  to as a "Partial Path".
+ Object Instance Path \- This is a Path Name to a Row in a Table in the
  ...
  #link(<sec:reference-following>)[Reference Following]). An Object
  Instance Path, as with an Object Path, is sometimes referred to as a
  "Partial Path".
...
#emph[Note: When resolving a Path Name, the Agent is expected to use
...
Agent would issue a network request in order to resolve a Path Name.];

#emph[Note: Obviously only one form of addressing or searching can be
...
=== 2.5.2 Using Instance Identifiers in Path Names <sec:using-instance-identifiers-in-path-names>

==== 2.5.2.1 Addressing by Instance Number <sec:addressing-by-instance-number>
...
#strong[#[R\-ARC.8]<r-arc.8>]; \- The assigned Instance Number MUST
persist unchanged until the Object Instance is subsequently deleted
(either by the USP Delete Message or through some external mechanism).
This implies that the Instance Number MUST persist across a reboot of
the Agent, and that the Agent MUST NOT allow the Instance Number of an
existing Object Instance to be modified by an external source.
...
#emph[Note: Controller and Agent interoperability is greatly affected by
an Agent’s implementation of Unique Keys. While this specification does
not aim to overlap its requirements to those of TR\-181
#link(<ref-TR-181>)[\[3\]], it is]; #strong[#emph[imperative];];
#emph[that an Agent implements Unique Keys for every Multi\-Instance
object in its Supported Data Model.];
...
#emph[Note: Addressing by Unique Key uses the same syntax as
...
rather than an address (which can’t match multiple instances).];
...
#strong[#[R\-ARC.9]<r-arc.9>]; \- An Agent MUST return Path Names that
...
concatenated by the AND (&&) logical operator #emph[(Note: the OR
...
Further, this Relative Path can’t include any child tables. #emph[(Note:
this is never necessary because any child tables that need to be
referenced in the Search Path can and should have their own
Expression)];
...
TR\-106 #link(<ref-TR-106>)[\[2\]]. #strong[For string and boolean
types, and also the Unknown Time dateTime value (cf.~TR\-106 Section
3.2.1 #link(<ref-TR-106>)[\[2\]]), only the '\=\=' and '!\=' operators
are valid.];
...
#emph[Note: Literal values are conceptually converted to a suitable
internal representation before comparison. For example, `int` values
`123`, `+123` and `0123` all represent the same value, and so do
`boolean` values `1` and `true`.];
...
#emph[Note: String values are enclosed in double quotes. In order to
...
character has to be quoted as %25.];
...
#strong[#[R\-ARC.9a]<r-arc.9a>]; \- Agents SHOULD tolerate whitespace on
either side of an Expression Operator.

#strong[#[R\-ARC.9b]<r-arc.9b>]; \- Controllers SHOULD NOT include
whitespace on either side of an Expression Operator.

===== 2.5.4.0.1 Search Expression Examples <sec:search-expression-examples>
...
#emph[Searches that are NOT VALID:];
...
#strong[#[R\-ARC.10]<r-arc.10>]; \- An Agent MUST return Path Names that
...
#emph[Note: It should be noted that according to the Device:2 Schema
#link(<ref-TR-106>)[\[2\]], reference Parameters:];

- #emph[Always contain Path Names (not Search Expressions)];
- #emph[When configured, can be configured using Path Names using
  Instance Number Addressing or Unique\-Key Addressing, however:];
- #emph[When the value of a reference Parameter is read, all Instance
  Identifiers are returned as Instance Numbers.];
#strong[#[R\-ARC.11]<r-arc.11>]; \- A USP Agent MUST support the ability
to use Key\-based addressing in reference values.
...
#strong[#[R\-ARC.12]<r-arc.12>]; \- The following requirements relate to
...
- To follow #emph[all]; references in the list, use a wildcard ("`*`")
...
==== 2.6.1.2 Search Expressions and Reference Following <sec:search-expressions-and-reference-following>
...
=== 2.6.2 Operations and Command Path Names <sec:operations-and-command-path-names>
...
=== 2.7.1 BNF Diagrams for Instantiated Data Model <sec:bnf-diagrams-for-instantiated-data-model>

#strong[#[idmpath]<bnf:idmpath>];:

#bbf-image("architecture/diagram/idmpath.png")
...
\
#strong[#[objpath]<bnf:objpath>];:

#bbf-image("architecture/diagram/objpath.png")
...
\
#strong[#[parampath]<bnf:parampath>];:

#bbf-image("architecture/diagram/parampath.png")
...
- #link(<bnf:idmpath>)[idmpath]
\
#strong[#[cmdpath]<bnf:cmdpath>];:

#bbf-image("architecture/diagram/cmdpath.png")
...
- #link(<bnf:idmpath>)[idmpath]
\
#strong[#[evntpath]<bnf:evntpath>];:

#bbf-image("architecture/diagram/evntpath.png")
...
- #link(<bnf:idmpath>)[idmpath]
\
#strong[#[inst]<bnf:inst>];:

#bbf-image("architecture/diagram/inst.png")
...
\
#strong[#[expr]<bnf:expr>];:

#bbf-image("architecture/diagram/expr.png")
...
- #link(<bnf:inst>)[inst]
\
#strong[#[exprcomp]<bnf:exprcomp>];:

#bbf-image("architecture/diagram/exprcomp.png")
...
- #link(<bnf:expr>)[expr]
\
#strong[#[relpath]<bnf:relpath>];:

#bbf-image("architecture/diagram/relpath.png")
...
\
#strong[#[reffollow]<bnf:reffollow>];:

#bbf-image("architecture/diagram/reffollow.png")
...
\
#strong[#[oper]<bnf:oper>];:

#bbf-image("architecture/diagram/oper.png")
...
- #link(<bnf:exprcomp>)[exprcomp]
\
#strong[#[value]<bnf:value>];:

#bbf-image("architecture/diagram/value.png")
...
- #link(<bnf:exprcomp>)[exprcomp]
\
#strong[#[name]<bnf:name>];:

#bbf-image("architecture/diagram/name.png")
...
- #link(<bnf:relpath>)[relpath]
\
#strong[#[literal]<bnf:literal>];:

#bbf-image("architecture/diagram/literal.png")
...
\
#strong[#[number]<bnf:number>];:

#bbf-image("architecture/diagram/number.png")
...
- #link(<bnf:value>)[value]
\
#strong[#[posnum]<bnf:posnum>];:

#bbf-image("architecture/diagram/posnum.png")
...
=== 2.7.2 BNF Diagrams for Supported Data Model <sec:bnf-diagrams-for-supported-data-model>

#strong[#[sdmpath]<bnf:sdmpath>];:

#bbf-image("architecture/diagram/sdmpath.png")
...
#strong[#[R\-DIS.0]<r-dis.0>]; \- An Agent that supports USP
configuration of Controllers MUST implement the
`Device.LocalAgent.Controller` Object as defined in the Device:2 Data
...
#link(<ref-RFC3925>)[\[14\]] (option code 125) and DHCPv6
Vendor\-specific Information Option #link(<ref-RFC3315>)[\[13\]] (option
code 17) can be used to provide information to Agents about a single
Controller. The options that may be returned by DNS are shown below.
Description of these options can be found in the Device:2 Data Model
#link(<ref-TR-181>)[\[3\]].

#strong[#[R\-DIS.1]<r-dis.1>]; \- If an Agent is configured to request
...
#strong[#[R\-DIS.1a]<r-dis.1a>]; \- The Agent MUST decode all received
options as strings (provisioning code, wait interval, and interval
multiplier are not decoded as numeric fields).

#strong[#[R\-DIS.1b]<r-dis.1b>]; \- The Agent MUST interpret a received
URL or FQDN of the Controller as either an absolute URL or FQDN.

#strong[#[R\-DIS.1c]<r-dis.1c>]; \- If the Agent receives an
encapsulated option value that is null terminated, the Agent MUST accept
the value provided, and MUST NOT interpret the null character as part of
the value.
...
#emph[Note: Requirement R\-DIS.2 was removed in USP 1.2.];
...
=== 3.3.1 DHCP Options for Controller Discovery <sec:dhcp-options-for-controller-discovery>
...
#[
  ...
  #table(
    ...
    [Parameter in the Device:2 Data Model #link(<ref-TR-181>)[\[3\]]
    ]),
    [URL or FQDN of the Controller
    ],
    [`25`
    ],
    [`25`
    ...
    [USP retry minimum wait interval
    ],
    [`27`
    ],
    [`27`
...
#strong[#[R\-DIS.2a]<r-dis.2a>]; \- When an Agent sends a DHCPv4
requests (DHCPDISCOVER, DHCPREQUEST, and DHCPINFORM) or DHCPv6 requests
(SOLICIT, REQUEST, RENEW, and INFORMATION\-REQUEST) it MUST include the
Encapsulation Options for requests below.

=== 3.4.2 DHCP Encapsulated Vendor\-Specific Option\-Data fields for DHCP requests <sec:dhcp-encapsulated-vendor-specific-option-data-fields-for-dhcp-requests>
...
#strong[#[R\-DIS.2c]<r-dis.2c>]; \- When an Agent receives a DHCPv4
...
#strong[#[R\-DIS.7]<r-dis.7>]; \- If the Agent is resolving an FQDN for
a Controller, and the MTP or resource path are unknown, the Agent MUST
...
#strong[#[R\-DIS.8]<r-dis.8>]; \- USP Endpoint DNS\-SD records MUST
include the USP Endpoint Identifier of the USP Endpoint as the DNS\-SD
...
=== 3.7.1 IANA\-Registered USP Service Names <sec:iana-registered-usp-service-names>
...
#strong[#[R\-DIS.9]<r-dis.9>]; \- Agent DNS\-SD records MUST include a
TXT record with the "path" and "name" attributes.

#strong[#[R\-DIS.10]<r-dis.10>]; \- The "name" attribute included in the
...
#strong[#[R\-DIS.11]<r-dis.11>]; \- Controller DNS\-SD records MUST
include a TXT record with the "path" attribute.

The "path" attribute is dependent on each #link(<sec:mtp>)[MTP].

#strong[#[R\-DIS.11a]<r-dis.11a>]; \- If a USP Endpoint requires MTP
...
=== 3.7.2 Example Controller Unicast DNS\-SD Resource Records <sec:example-controller-unicast-dns-sd-resource-records>
...
=== 3.7.3 Example Agent Multicast DNS\-SD Resource Records <sec:example-agent-multicast-dns-sd-resource-records>
...
=== 3.7.4 Example Controller Multicast DNS\-SD Resource Records <sec:example-controller-multicast-dns-sd-resource-records>
...
== 3.8 Using the SendOnBoardRequest() operation and OnBoardRequest notification <sec:using-the-sendonboardrequest-operation-and-onboardrequest-notification>
...
#emph[Note: Message Transfer Protocol was a term adopted to avoid
...
Protocol (MTP) refers to application layer transport.];
...
#strong[#[R\-MTP.0]<r-mtp.0>]; – The Message Transfer Protocol MUST use
...
#strong[#[R\-MTP.1]<r-mtp.1>]; – When TLS is used to secure an MTP, an
Agent MUST require the MTP peer to provide an X.509 certificate.

#strong[#[R\-MTP.2]<r-mtp.2>]; \- An Agent capable of obtaining absolute
...
#strong[#[R\-MTP.3]<r-mtp.3>]; \- An Agent that has obtained an accurate
absolute time MUST validate those components of the received X.509
certificate that involve absolute time.

#strong[#[R\-MTP.4]<r-mtp.4>]; \- When an Agent receives an X.509
...
#strong[#[R\-MTP.4a]<r-mtp.4a>]; \- When an Agent receives an X.509
...
#figure(
  ...
  #bbf-image("mtp/validate-cert.png")<img:receiving-a-x.509-certificate>
] <fig:receiving-a-x509-certificate>
#emph[Note: The .local and .home.arpa domains are defined by the IETF as
...
#strong[#[R\-MTP.4b]<r-mtp.4b>]; \- Any field that is noted as
"Required" in its description MUST be sent.
...
#emph[Note: This version of the specification defines the USP Record in
...
non\-normative) if further encodings are specified in future versions.];

`string version`

Required. Version (Major.Minor) of the USP Protocol (e.g., "1.3").

#emph[Note: The version field is used for USP Endpoints to set
expectations about their behavior for other USP Endpoints.];
#strong[#emph[A USP Endpoint that receives a Record indicating a version
higher than it supports can expect to receive messages it may not
understand or encounter other unexpected behavior.];]; #emph[USP
Endpoints are expected to handle these inconsistencies gracefully (For
example, using the 7001 Message Not Supported Error).];
...
===== 4.1.3.1.1 NoSessionContextRecord fields <sec:nosessioncontextrecord-fields>
...
===== 4.1.3.1.2 SessionContextRecord fields <sec:sessioncontextrecord-fields>
...
#emph[Note: Endpoints maintain independent values for received and sent
sequence\_id for a Session Context, based respectively on the number of
received and sent Records.];
...
===== 4.1.3.1.3 WebSocketConnectRecord fields <sec:websocketconnectrecord-fields>
...
===== 4.1.3.1.5 STOMPConnectRecord fields <sec:stompconnectrecord-fields>
...
#strong[#[R\-MTP.5]<r-mtp.5>]; \- A recipient of an erroneous USP Record
...
#[
  ...
  #table(
    ...
    [This error indicates the received USP Record could not be parsed.
    ],
    [`7101`
    ],
    [Secure session required
    ],
    [This error indicates USP layer
    #link(<sec:secure-message-exchange>)[Secure Message Exchange] is
    required.
    ],
    [`7102`
    ],
    [Secure session not supported
    ],
    [This error indicates USP layer
    #link(<sec:secure-message-exchange>)[Secure Message Exchange] was
    indicated in the received Record but is not supported by the
    receiving Endpoint.
    ],
    [`7103`
    ],
    [Segmentation and reassembly not supported
    ],
    [This error indicates segmentation and reassembly was indicated in
    the received Record but is not supported by the receiving Endpoint.
    ],
    [`7104`
    ],
    [Invalid Record value
    ],
    [This error indicates the value of at least one Record field was
    invalid.
    ],
    [`7105`
    ],
    ...
    [This error indicates an existing Session Context
    #link(<sec:establishing-an-e2e-session-context>)[Establishing an E2E Session Context]
    is being terminated.
    ],
    [`7106`
    ],
    [Session Context not allowed
    ],
    [This error indicates use of Session Context
    #link(<sec:establishing-an-e2e-session-context>)[Establishing an E2E Session Context]
...
=== 4.1.5 Connect and Disconnect Record Types <sec:connect-and-disconnect-record-types>
...
#strong[#[R\-MTP.6]<r-mtp.6>]; \- If a USP Agent has the necessary
...
#strong[#[R\-MTP.7]<r-mtp.7>]; \- The USP Agent SHOULD send a
...
#strong[#[R\-MTP.8]<r-mtp.8>]; \- After sending or receiving a
...
#strong[#[R\-MTP.9]<r-mtp.9>]; \- A `DisconnectRecord` SHOULD include
the `reason_code` and `reason` fields with an applicable code from
#link(<sec:usp-record-errors>)[USP Record Errors].

== 4.2 CoAP Binding (OBSOLETED) <sec:coap-binding-obsoleted>

#emph[Note: The CoAP MTP was deprecated in USP 1.2. Due to the way it is
...
#emph[Note: The CoAP MTP was obsoleted in USP 1.3 as a natural
progression from being deprecated in USP 1.2.];
...
#figure(
  ...
  #bbf-image("mtp/coap/usp-request-response-over-coap.png")<img:example-usp-requestresponse-over-the-coap-mtp>
...
#strong[#[R\-COAP.2]<r-coap.2>]; \- A CoAP request message MUST include
...
CoAP request message it MUST use the reply\-to Uri\-Query option
included in the CoAP request as the CoAP URI for the USP Response (if a
response is required by the incoming USP Request).

#strong[#[R\-COAP.3]<r-coap.3>]; \- When creating DNS\-SD records (see
...
=== 4.2.2 Mapping USP Records to CoAP Messages <sec:mapping-usp-records-to-coap-messages>
...
#strong[#[R\-COAP.6]<r-coap.6>]; \- The CoAP Content\-Format for USP
Records MUST be application\/octet\-stream (ID\=42) for
#link(<sec:encoding>)[Message Encoding].

#strong[#[R\-COAP.7]<r-coap.7>]; \- Upon successful reception of the
CoAP message using POST, the CoAP server MUST respond with a response
code of `2.04 (Changed)`.

==== 4.2.2.1 Handling CoAP Request Failures <sec:handling-coap-request-failures>
...
the CoAP client MUST indicate a timeout to the USP Endpoint.

#strong[#[R\-COAP.10]<r-coap.10>]; \- When a CoAP client receives a
response code of 4.xx or 5.xx, the CoAP client MUST indicate a CoAP
failure to the USP Endpoint.
...
- Invalid or not understandable payload: The CoAP server responds with a
  `4.00`
#strong[#[R\-COAP.11]<r-coap.11>]; \- When a CoAP server receives a CoAP
request with an invalid CoAP method, the CoAP server MUST respond with a
`4.05` response code.

#strong[#[R\-COAP.12]<r-coap.12>]; \- When a CoAP server receives a CoAP
request with an invalid CoAP Content\-Format option, the CoAP server
MUST respond with a `4.15` response code.

#strong[#[R\-COAP.13]<r-coap.13>]; \- When a CoAP server receives a CoAP
...
#strong[#[R\-COAP.16]<r-coap.16>]; – When the Certificate mode of CoAP
is used to secure an MTP, a USP Endpoint MUST provide an X.509
certificate to the MTP peer.
...
=== 4.3.1 Mapping USP Endpoints to WebSocket URIs <sec:mapping-usp-endpoints-to-websocket-uris>
...
#figure(
  ...
  #bbf-image("mtp/websocket/websocket-session-handshake.png")<img:websocket-session-handshake>
...
#strong[#[R\-WS.6]<r-ws.6>]; \- USP Agent MUST provide the capability to
accept the establishment of a WebSocket session from a USP Controller.

#emph[Note: Requirement #link(<r-ws.6>)[R\-WS.6] was altered from a MAY
to a MUST in USP 1.2 to ensure that the WebSocket MTP is suitable for
the in\-home communications use case.];
...
#emph[Note: USP Endpoint IDs can contain percent\-encoded characters,
which can create confusion when encoding it for URI use. The Endpoint ID
needs to be treated as opaque data for it to be properly "roundtripable"
when encoded\/decoded for URI use, thus any percent character needs to
be percent\-encoded, e.g.~`doc::agent%21` will be encoded as
`doc::agent%2521`.];

#strong[#[R\-WS.9]<r-ws.9>]; \- The WebSocket’s handshake
`Sec-WebSocket-Protocol` header for exchange of USP Records using the
protocol\-buffers encoding mechanism MUST be `v1.usp`.

#strong[#[R\-WS.10]<r-ws.10>]; \- A WebSocket client MUST include the
`Sec-WebSocket-Protocol` header for exchange of USP Records when
initiating a WebSocket session.

#strong[#[R\-WS.10a]<r-ws.10a>]; (DEPRECATED) \- A WebSocket client MUST
...
#emph[Note: Requirement #link(<r-ws.10a>)[R\-WS.10a] was removed in USP
1.3, due to the impossibility of setting WebSocket Extensions in some
environments.];
...
#strong[#[R\-WS.10c]<r-ws.10c>]; \- When encoding the USP Endpoint ID
for URI use, it MUST be treated as opaque data and any percent character
MUST itself be percent\-encoded, i.e.~as `%25`.

#strong[#[R\-WS.11]<r-ws.11>]; \- A WebSocket server that supports USP
...
#strong[#[R\-WS.11a]<r-ws.11a>]; \- A WebSocket server SHOULD include
the `Sec-WebSocket-Extensions` header with `bbf-usp-protocol` WebSocket
...
#strong[#[R\-WS.11b]<r-ws.11b>]; \- WebSocket clients MUST NOT consider
WebSocket responses that do not include the `bbf-usp-protocol` WebSocket
Extension to be an error.

#strong[#[R\-WS.11c]<r-ws.11c>]; \- WebSocket servers MUST NOT consider
WebSocket session initiation requests that do not include the
`bbf-usp-protocol` WebSocket Extension to be an error.

#strong[#[R\-WS.12]<r-ws.12>]; \- A WebSocket client MUST NOT establish
a WebSocket session if the response to a WebSocket session initiation
...
#strong[#[R\-WS.12a]<r-ws.12a>]; \- A WebSocket server MUST NOT
establish a WebSocket session if the WebSocket session initiation
request does not include the `Sec-WebSocket-Protocol` header.

=== 4.3.3 Handling of WebSocket Frames <sec:handling-of-websocket-frames>
...
figure below. WebSocket message fragmentation should be avoided, because
its main benefit does not apply for the case of sending USP Records.

#figure(
  caption: [Figure 5 – USP Request using a WebSocket Session
    ])[
  #bbf-image("mtp/websocket/USP-request-over-websocket.png")<img:usp-request-using-a-websocket-session>
...
#strong[#[R\-WS.14b]<r-ws.14b>]; \- USP Endpoints MUST support WebSocket
message fragmentation in received USP Records.
...
#strong[#[R\-WS.16]<r-ws.16>]; \- A USP Endpoint that receives a
WebSocket frame containing a USP Record that cannot be extracted for
processing, the receiving USP Endpoint MUST terminate the connection
using a WebSocket Close frame with a Status Code of `1003`.

==== 4.3.3.2 Keeping the WebSocket Session Alive <sec:keeping-the-websocket-session-alive>
...
control frames to the remote USP Endpoint.
...
#strong[#[R\-WS.19]<r-ws.19>]; – When retrying to establish a WebSocket
...
#[
  ...
  #table(
    ...
    [Default Wait Interval Range (min\-max seconds)
    ],
    [Actual Wait Interval Range (min\-max seconds)
    ]),
    [\#1
    ],
    [5\-10
    ...
    [m.(k\/1000)^2 \- m.(k\/1000)^3
    ],
    [\#4
    ],
    [40\-80
    ],
    [m.(k\/1000)^3 \- m.(k\/1000)^4
    ],
    [\#5
    ],
    [80\-160
    ],
    [m.(k\/1000)^4 \- m.(k\/1000)^5
    ],
    [\#6
    ],
    [160\-320
    ],
    [m.(k\/1000)^5 \- m.(k\/1000)^6
    ],
    [\#7
    ],
    [320\-640
    ],
    [m.(k\/1000)^6 \- m.(k\/1000)^7
    ],
    [\#8
    ],
    [640\-1280
    ],
    [m.(k\/1000)^7 \- m.(k\/1000)^8
    ],
    [\#9
    ],
    [1280\-2560
    ],
    [m.(k\/1000)^8 \- m.(k\/1000)^9
    ],
    [\#10 and subsequent
    ],
    [2560\-5120
    ],
    [m.(k\/1000)^9 \- m.(k\/1000)^10
    ]
  )
]

#strong[#[R\-WS.20]<r-ws.20>]; – Once a WebSocket session is established
...
#strong[#[R\-WS.21]<r-ws.21>]; – If a reboot of the Agent occurs, the
Agent MUST reset the WebSocket MTP’s retry count to zero for the next
...
1.2 of the STOMP protocol #link(<ref-STOMP-1-2>)[\[40\]], further
referred to as "STOMP Specification", or the Simple Text Oriented
Message Protocol. Messages that are transferred between STOMP clients
utilize a message bus interaction model where the STOMP server is the
messaging broker that routes and delivers messages based on the
destination included in the STOMP header.
...
#figure(
  ...
  #bbf-image("mtp/stomp/STOMP-architecture.jpg")<img:usp-over-stomp-architecture>
] <fig:usp-over-stomp>
...
+ Send USP Records
#strong[#[R\-STOMP.0]<r-stomp.0>]; \- USP Agents utilizing STOMP clients
for message transport MUST support the `STOMPConn:1` and
`STOMPController:1` data model profiles.

#strong[#[R\-STOMP.1]<r-stomp.1>]; \- USP Agents utilizing STOMP clients
for message transport SHOULD support the `STOMPAgent:1` and
`STOMPHeartbeat:1` data model profile.

=== 4.4.1 Handling of the STOMP Session <sec:handling-of-the-stomp-session>
...
==== 4.4.1.1 Connecting a USP Endpoint to the STOMP Server <sec:connecting-a-usp-endpoint-to-the-stomp-server>

#strong[#[R\-STOMP.2]<r-stomp.2>]; \- USP Endpoints utilizing STOMP
clients for message transport MUST send a `STOMP` frame to the STOMP
server to initiate the STOMP communications session as defined in the
"Connecting" section of the STOMP Specification.

#strong[#[R\-STOMP.3]<r-stomp.3>]; \- USP Endpoints that DO NOT utilize
...
#strong[#[R\-STOMP.4]<r-stomp.4>]; \- USP Endpoints sending a `STOMP`
frame MUST include (in addition to other mandatory STOMP headers) an
`endpoint-id` STOMP header containing the Endpoint ID of the USP
Endpoint sending the frame. #emph[Note: According to the STOMP
Specification, the `STOMP` frame requires that "C style literal escapes"
need to be used to encode any carriage return, line feed, or colon
characters that are found within the UTF\-8 encoded headers, and
...
characters, those will need to be escaped.];

#strong[#[R\-STOMP.5]<r-stomp.5>]; \- USP Endpoints sending a STOMP
frame MUST include a host STOMP header, if configured to do so. For a
USP Agent the value MUST contain the value from the appropriate
`.STOMP.Connection.{i}.VirtualHost` Parameter if supported and not
empty.

#strong[#[R\-STOMP.6]<r-stomp.6>]; \- If the USP Endpoint receives a
...
#strong[#[R\-STOMP.7]<r-stomp.7>]; \- If the connection to the STOMP
server is NOT successful then the USP Endpoint MUST enter a connection
...
==== 4.4.1.2 Handling the STOMP Heart Beat Mechanism <sec:handling-the-stomp-heart-beat-mechanism>
...
#strong[#[R\-STOMP.8]<r-stomp.8>]; \- If the `STOMP.Connection`
instance’s `EnableHeartbeats` Parameter value is true then the USP Agent
MUST negotiate the STOMP Heart Beat mechanism within the `STOMP` frame
during the process of establishing the STOMP connection as is defined in
the "Heart\-beating" section of the STOMP Specification.

#strong[#[R\-STOMP.9]<r-stomp.9>]; \- If the `STOMP.Connection`
instance’s `EnableHeartbeats` Parameter value is either false or not
implemented then the USP Agent MUST either not send the `heart-beat`
STOMP header in the `STOMP` frame or send "0,0" as the value of the
`heart-beat` STOMP header in the `STOMP` frame.

#strong[#[R\-STOMP.10]<r-stomp.10>]; \- USP Agents negotiating the STOMP
...
#strong[#[R\-STOMP.11]<r-stomp.11>]; \- USP Agents that have negotiated
a STOMP Heart Beat mechanism with a STOMP server MUST adhere to the
heart beat values (as defined in the "Heart\-beating" section of the
STOMP Specification) as returned in the `CONNECTED` frame.

=== 4.4.2 Mapping USP Endpoints to STOMP Destinations <sec:mapping-usp-endpoints-to-stomp-destinations>
...
==== 4.4.2.1 Subscribing a USP Endpoint to a STOMP Destination <sec:subscribing-a-usp-endpoint-to-a-stomp-destination>

#strong[#[R\-STOMP.12]<r-stomp.12>]; \- USP Endpoints utilizing STOMP
...
#strong[#[R\-STOMP.13]<r-stomp.13>]; \- USP Endpoints sending a
`SUBSCRIBE` frame MUST include (in addition to other mandatory STOMP
headers) a `destination` STOMP header containing the STOMP destination
associated with the USP Endpoint sending the frame.

#strong[#[R\-STOMP.14]<r-stomp.14>]; \- USP Agents that receive a
...
#strong[#[R\-STOMP.15]<r-stomp.15>]; \- USP Agents that have NOT
received a `subscribe-dest` STOMP Header in the `CONNECTED` frame MUST
use the STOMP destination found in the
`Device.LocalAgent.MTP.{i}.STOMP.Destination` Parameter in the
`destination` STOMP header when sending a `SUBSCRIBE` frame.

#strong[#[R\-STOMP.16]<r-stomp.16>]; \- USP Agents that have NOT
received a `subscribe-dest` STOMP Header in the `CONNECTED` frame and do
NOT have a value in the `Device.LocalAgent.MTP.{i}.STOMP.Destination`
Parameter MUST terminate the STOMP communications session (preferably
via the `DISCONNECT` frame) and enter a connection retry state following
#link(<r-stomp.7>)[R\-STOMP.7].

#strong[#[R\-STOMP.17]<r-stomp.17>]; \- USP Endpoints sending a
`SUBSCRIBE` frame MUST use an `ack` value of "auto".

=== 4.4.3 Mapping USP Records to STOMP Frames <sec:mapping-usp-records-to-stomp-frames>
...
#strong[#[R\-STOMP.18]<r-stomp.18>]; \- USP Endpoints utilizing STOMP
...
#strong[#[R\-STOMP.19]<r-stomp.19>]; \- USP Endpoints sending a `SEND`
...
#strong[#[R\-STOMP.20]<r-stomp.20>]; \- USP Endpoints sending a `SEND`
frame MUST include (in addition to other mandatory STOMP headers) a
...
#strong[#[R\-STOMP.21]<r-stomp.21>]; \- USP Endpoints sending a `SEND`
frame with content\-type of `application/vnd.bbf.usp.msg` MUST include
(in addition to other mandatory STOMP headers) a `reply-to-dest` STOMP
...
#strong[#[R\-STOMP.22]<r-stomp.22>]; \- USP Endpoints sending a `SEND`
frame with content\-type of `application/vnd.bbf.usp.msg` MUST include
the Protocol Buffer #link(<ref-PROTOBUF>)[\[4\]] binary encoding of the
USP Record as the body of the `SEND` frame.

#strong[#[R\-STOMP.23]<r-stomp.23>]; \- When a USP Endpoint receives a
...
#emph[Note: Error handling was unified between MTPs in USP 1.2 by using
USP Records instead of MTP specific messages, deprecating most of this
section, specifically the requirements
#strong[#link(<r-stomp.23a>)[R\-STOMP.23a]];,
#strong[#link(<r-stomp.23b>)[R\-STOMP.23b]];,
#strong[#link(<r-stomp.24>)[R\-STOMP.24]];,
#strong[#link(<r-stomp.24a>)[R\-STOMP.24a]]; and
#strong[#link(<r-stomp.24b>)[R\-STOMP.24b]];. Please see
#link(<sec:usp-record-errors>)[USP Record Errors] for details.];

#strong[#[R\-STOMP.23a]<r-stomp.23a>]; (DEPRECATED) \- USP Endpoints
MUST support STOMP content\-type header value of
`application/vnd.bbf.usp.error`.

#emph[Note: Requirement #link(<r-stomp.23a>)[R\-STOMP.23a] was removed
in USP 1.2];

#strong[#[R\-STOMP.23b]<r-stomp.23b>]; (DEPRECATED) \- A USP Endpoint
MUST include a `usp-err-id` STOMP header in `SEND` frames of
content\-type `application/vnd.bbf.usp.msg`. The value of this header
is: `<USP Record to_id> + "/" + <USP Message msg_id>`, the
...
#emph[Note: Requirement #link(<r-stomp.23b>)[R\-STOMP.23b] was removed
in USP 1.2];

#strong[#[R\-STOMP.24]<r-stomp.24>]; (DEPRECATED) \- When a USP Endpoint
...
#emph[Note: Requirement #link(<r-stomp.24>)[R\-STOMP.24] was removed in
USP 1.2];

#strong[#[R\-STOMP.24a]<r-stomp.24a>]; (DEPRECATED) \- When a USP
Endpoint receives a MESSAGE frame containing a USP Record or an
...
with an `application/vnd.bbf.usp.error` content\-type header value if
the received STOMP MESSAGE frame included a `usp-err-id` header.

#emph[Note: Requirement #link(<r-stomp.24a>)[R\-STOMP.24a] was removed
in USP 1.2];

#strong[#[R\-STOMP.24b]<r-stomp.24b>]; (DEPRECATED) \- A STOMP SEND
frame with `application/vnd.bbf.usp.error` content\-type MUST contain
the received `usp-err-id` header, the destination header value set to
the received `reply-to-dest` header, and a message body (formatted using
...
```
#emph[Note: Requirement #link(<r-stomp.24b>)[R\-STOMP.24b] was removed
in USP 1.2];

#strong[#[R\-STOMP.25]<r-stomp.25>]; \- If an `ERROR` frame is received
by the USP Endpoint, the STOMP server will terminate the connection. In
...
==== 4.4.3.2 Handling Other STOMP Frames <sec:handling-other-stomp-frames>

#strong[#[R\-STOMP.26]<r-stomp.26>]; \- USP Endpoints utilizing STOMP
clients for message transport MUST NOT send the transactional STOMP
frames including: `BEGIN`, `COMMIT`, and `ABORT`.

#strong[#[R\-STOMP.27]<r-stomp.27>]; \- USP Endpoints utilizing STOMP
clients for message transport MUST NOT send the acknowledgement STOMP
frames including: `ACK` and `NACK`.

#strong[#[R\-STOMP.28]<r-stomp.28>]; \- USP Endpoints utilizing STOMP
...
#strong[#[R\-STOMP.29]<r-stomp.29>]; \- USP Endpoints utilizing STOMP
...
#strong[#[R\-STOMP.30]<r-stomp.30>]; \- USP Endpoints utilizing STOMP
...
#strong[#[R\-STOMP.31]<r-stomp.31>]; \- When creating a DNS\-SD record,
an Endpoint MUST set the DNS\-SD "`path`" attribute equal to the value
of the destination that it has subscribed to.

#strong[#[R\-STOMP.32]<r-stomp.32>]; \- When creating a DNS\-SD record,
an Endpoint MUST utilize the STOMP server’s address information in the A
...
#strong[#[R\-STOMP.33]<r-stomp.33>]; \- A STOMP server implementation
MUST adhere to the requirements defined in the STOMP Specification.

#strong[#[R\-STOMP.34]<r-stomp.34>]; \- A STOMP server implementation
MUST perform authentication of the STOMP client and ensure that a Remote
USP Endpoint is only allowed to subscribe to the destination that is
associated with the USP Endpoint.

#strong[#[R\-STOMP.35]<r-stomp.35>]; \- A STOMP server implementation
SHOULD support both Client Certification Authentication and
Username\/Password Authentication mechanisms.
...
#figure(
  ...
  #bbf-image("mtp/mqtt/mqtt-architecture.png")<img:usp-over-mqtt-architecture>
] <fig:usp-over-mqtt>
...
#figure(
  ...
  #bbf-image("mtp/mqtt/mqtt-packets.png")<img:mqtt-packets>
] <fig:mqtt-packets>
...
#strong[#[R\-MQTT.3]<r-mqtt.3>]; \- USP Agents utilizing MQTT clients
for message transport MUST support MQTT over TCP transport protocol.
...
for message transport SHOULD support the `MQTTClientExtended:1` data
model profile.

=== 4.5.1 Connecting a USP Endpoint to the MQTT Server <sec:connecting-a-usp-endpoint-to-the-mqtt-server>
...
#strong[#[R\-MQTT.6]<r-mqtt.6>]; \- USP Endpoints utilizing MQTT clients
for message transport MUST send a `CONNECT` packet to the MQTT server to
initiate the MQTT communications session.

#strong[#[R\-MQTT.7]<r-mqtt.7>]; \- USP Endpoints with a configured MQTT
...
#strong[#[R\-MQTT.8]<r-mqtt.8>]; \- USP Endpoints MUST set the value of
the `CONNECT` packet Client Identifier (ClientId) as follows:
...
#strong[#[R\-MQTT.9]<r-mqtt.9>]; \- An MQTT 5.0 client MUST save (in the
...
#strong[#[R\-MQTT.10]<r-mqtt.10>]; \- If the connection to the MQTT
server is NOT successful then the USP Endpoint MUST enter a connection
retry state. For a USP Agent the retry mechanism is based on the
`MQTT.Client.{i}.` retry Parameters: `ConnectRetryTime`,
`ConnectRetryIntervalMultiplier`, and `ConnectRetryMaxInterval`.

#strong[#[R\-MQTT.11]<r-mqtt.11>]; \- Once a USP Endpoint has
successfully connected to an MQTT server, it MUST use the same ClientId
for all subsequent connections with that server.

==== 4.5.1.1 CONNECT Flags and Properties <sec:connect-flags-and-properties>
...
#strong[#[R\-MQTT.12]<r-mqtt.12>]; \- An MQTT 5.0 USP Endpoint MUST
support setting the Request Response Information property to 1, and MUST
support receiving the corresponding Response Information in the
`CONNACK` packet.
...
#strong[#[R\-MQTT.13]<r-mqtt.13>]; \- An MQTT 5.0 USP Endpoint MUST
include a User Property name\-value pair in the `CONNECT` packet with
name of "usp\-endpoint\-id" and value of this Endpoint’s USP Endpoint
ID.
...
#strong[#[R\-MQTT.14]<r-mqtt.14>]; \- USP Endpoints with a configured
Keep Alive value MUST include this in the MQTT `CONNECT` packet. The
...
#[R\-MQTT.1]<r-mqtt.1-1> and #link(<r-mqtt.2>)[R\-MQTT.2] require that
all MQTT capabilities referenced in this section and its sub\-sections
are compliant with the MQTT specifications. Reading the MQTT
specification is highly recommended to ensure the correct syntax and
usage of MQTT packets and properties (`SUBSCRIBE`, Topic Filter, QoS 0,
QoS 1, QoS 2, etc.).

#strong[#[R\-MQTT.15]<r-mqtt.15>]; \- USP Endpoints that successfully
...
#strong[#[R\-MQTT.16]<r-mqtt.16>]; \- USP Agents that have NOT received
any Subscriptions outlined in #link(<r-mqtt.15>)[R\-MQTT.15]
...
#strong[#[R\-MQTT.17]<r-mqtt.17>]; \- If a USP Endpoint does not
...
setting to use. In order to ensure deployments have the ability to use
at least QoS 1, MQTT clients and servers are required to implement at
least QoS 1 (see requirements in
#link(<sec:sending-the-usp-record-in-a-publish-packet-payload>)[Sending the USP Record in a PUBLISH Packet Payload]
and #link(<sec:mqtt-server-requirements>)[MQTT Server Requirements]).

=== 4.5.3 Sending the USP Record in a PUBLISH Packet Payload <sec:sending-the-usp-record-in-a-publish-packet-payload>
...
#[R\-MQTT.1]<r-mqtt.1-2> and #link(<r-mqtt.2>)[R\-MQTT.2] require that
all MQTT capabilities referenced in this section and its sub\-sections
are compliant with the MQTT specifications. Reading the MQTT
specification is highly recommended to ensure the correct syntax and
usage of MQTT packets and properties (`PUBLISH`, Content Type, Response
Topic, etc.).

#strong[#[R\-MQTT.18]<r-mqtt.18>]; \- USP Endpoints utilizing MQTT
clients for message transport MUST send the USP Record in the payload of
a `PUBLISH` packet.

#strong[#[R\-MQTT.19]<r-mqtt.19>]; \- USP Endpoints MUST send USP
Records using the Protocol Buffer #link(<ref-PROTOBUF>)[\[4\]] binary
encoding of the USP Record.

#strong[#[R\-MQTT.20]<r-mqtt.20>]; \- USP Endpoints utilizing MQTT
clients for message transport MUST support MQTT QoS 0 and QoS 1.
...
#strong[#[R\-MQTT.21]<r-mqtt.21>]; \- An MQTT 5.0 USP Endpoint that
receives Response Information in the `CONNACK` packet MUST use this as
its "reply to" Topic.

#emph[Note: By using a single "reply to" Topic for all USP connections,
...
#strong[#[R\-MQTT.22]<r-mqtt.22>]; \- USP Endpoints MUST include a
"reply to" Topic in all `PUBLISH` packets transporting USP Records.

#strong[#[R\-MQTT.23]<r-mqtt.23>]; \- USP Endpoints using MQTT 5.0 MUST
include their "reply to" Topic in the `PUBLISH` Response Topic property.

#strong[#[R\-MQTT.24]<r-mqtt.24>]; \- USP Endpoints using MQTT 3.1.1
MUST include their "reply to" Topic after "\/reply\-to\=" at the end of
the `PUBLISH` Topic Name, with any "\/" character in the Topic replaced
by "%2F".
...
#strong[#[R\-MQTT.25]<r-mqtt.25>]; \- USP Endpoints using MQTT 3.1.1
MUST interpret the portion of the received `PUBLISH` Topic Name
following the last forward slash "\/reply\-to\=" as the response Topic
Name. Any instance of "%2F" in this received string MUST be replaced
with "\/".

#strong[#[R\-MQTT.26]<r-mqtt.26>]; \- USP Endpoints using MQTT 5.0 MUST
use the received `PUBLISH` Response Topic property as the response Topic
Name.
...
#strong[#[R\-MQTT.26a]<r-mqtt.26a>]; \- If a MQTT version mismatch is
...
#strong[#[R\-MQTT.27]<r-mqtt.27>]; \- USP Endpoints sending a USP Record
...
#strong[#[R\-MQTT.27a]<r-mqtt.27a>]; \- USP Endpoints receiving a MQTT
5.0 `PUBLISH` packet MUST interpret the Content Type property of
"usp.msg" or "application\/vnd.bbf.usp.msg" (the registered USP Record
MIME type) as indicating the packet contains a USP Record.
...
#emph[Note: Error handling was unified between MTPs in USP 1.2 by using
USP Records instead of MTP specific messages, deprecating most of this
section, specifically #strong[#link(<r-mqtt.28>)[R\-MQTT.28]];,
#strong[#link(<r-mqtt.29>)[R\-MQTT.29]];,
#strong[#link(<r-mqtt.30>)[R\-MQTT.30]];,
#strong[#link(<r-mqtt.31>)[R\-MQTT.31]];,
#strong[#link(<r-mqtt.31a>)[R\-MQTT.31a]]; and
#strong[#link(<r-mqtt.32>)[R\-MQTT.32]];. Please see
#link(<sec:usp-record-errors>)[USP Record Errors] for details.];

#strong[#[R\-MQTT.28]<r-mqtt.28>]; (DEPRECATED) \- MQTT 5.0 Endpoints
MUST support `PUBLISH` Content Type value of "usp.error".

#emph[Note: Requirement #link(<r-mqtt.28>)[R\-MQTT.28] was removed in
USP 1.2];

#strong[#[R\-MQTT.29]<r-mqtt.29>]; (DEPRECATED) \- MQTT 5.0 Endpoints
MUST include a `usp-err-id` MQTT User Property in `PUBLISH` packets of
content\-type "usp.msg". The value of this User Property is:
`<USP Record to_id> + "/" + <USP Message msg_id>`, the
`<USP Message msg_id>` field can be left blank if the Record does not
contain a USP Message.

#emph[Note: Requirement #link(<r-mqtt.29>)[R\-MQTT.29] was removed in
USP 1.2];

#strong[#[R\-MQTT.30]<r-mqtt.30>]; (DEPRECATED) \- When an MQTT 3.1.1
USP Endpoint receives a `PUBLISH` packet containing a USP Record or an
encapsulated USP Message within a USP Record that cannot be extracted
for processing, the receiving USP Endpoint MUST silently drop the USP
Record.

#emph[Note: Requirement #link(<r-mqtt.30>)[R\-MQTT.30] was removed in
USP 1.2];

#strong[#[R\-MQTT.31]<r-mqtt.31>]; (DEPRECATED) \- When an MQTT 5.0 USP
...
#emph[Note: Requirement #link(<r-mqtt.31>)[R\-MQTT.31] was removed in
USP 1.2];

#strong[#[R\-MQTT.31a]<r-mqtt.31a>]; (DEPRECATED) \- USP Endpoints
receiving a MQTT 5.0 `PUBLISH` packet MUST interpret the Content Type
property of "usp.error" or "application\/vnd.bbf.usp.error" (the
registered USP error message MIME type) as indicating the packet
contains a USP error message and code.

#emph[Note: Requirement #link(<r-mqtt.31a>)[R\-MQTT.31a] was removed in
USP 1.2];

#strong[#[R\-MQTT.32]<r-mqtt.32>]; (DEPRECATED) \- When a USP Endpoint
using MQTT 5.0 receives a `PUBLISH` packet with QoS 1 or QoS 2
...
Endpoint MUST include Reason Code 153 (0x99) identifying "Payload format
invalid" in any `PUBACK` or `PUBREC` packet.
...
#emph[Note: Requirement #link(<r-mqtt.32>)[R\-MQTT.32] was removed in
USP 1.2];

#strong[#[R\-MQTT.33]<r-mqtt.33>]; \- If the MQTT server terminates the
...
#strong[#[R\-MQTT.34]<r-mqtt.34>]; \- USP Endpoints utilizing MQTT
clients for message transport SHOULD send an `UNSUBSCRIBE` packet when a
subscribed Topic or Topic Filter is no longer indicated but the MQTT
connection is expected to stay up.

#[R\-MQTT.1]<r-mqtt.1-3> and #link(<r-mqtt.2>)[R\-MQTT.2] require that
all MQTT capabilities referenced in this section and its sub\-sections
are compliant with the MQTT specifications. Reading the MQTT
specification is highly recommended to ensure the correct syntax and
usage of MQTT packets and properties (`DISCONNECT`, etc.).

#strong[#[R\-MQTT.35]<r-mqtt.35>]; \- USP Endpoints utilizing MQTT
clients for message transport SHOULD send a `DISCONNECT` packet when
shutting down an MQTT connection.
...
#strong[#[R\-MQTT.36]<r-mqtt.36>]; \- When creating a DNS\-SD record
...
#strong[#[R\-MQTT.37]<r-mqtt.37>]; \- When creating a DNS\-SD record
...
#strong[#[R\-MQTT.38]<r-mqtt.38>]; \- When creating a DNS\-SD record, an
...
#strong[#[R\-MQTT.42]<r-mqtt.42>]; \- An MQTT server MUST support
...
#strong[#[R\-MQTT.43]<r-mqtt.43>]; \- An MQTT server SHOULD support both
Client Certification Authentication and User Name \/ Password
Authentication mechanisms.

#strong[#[R\-MQTT.44]<r-mqtt.44>]; \- An MQTT server SHOULD support
sending Topic or Topic Filter values in a "subscribe\-topic" User
Property in the `CONNACK` packet.

#strong[#[R\-MQTT.45]<r-mqtt.45>]; \- If an MQTT server supports
...
#strong[#[R\-MQTT.46]<r-mqtt.46>]; \- An MQTT server MUST support at
least MQTT QoS 1 level.

#strong[#[R\-MQTT.47]<r-mqtt.47>]; \- An MQTT server SHOULD support a
...
#figure(
  ...
  #bbf-image("mtp/unix-domain-socket/unix-domain-socket-binding.png")<img:unix-domain-socket-binding>
] <fig:unix-domain-socket-binding>

=== 4.6.1 Handling UNIX Domain Socket Connections <sec:handling-unix-domain-socket-connections>
...
#strong[#[R\-UDS.1]<r-uds.1>]; \- USP Agents utilizing UNIX domain
socket servers or clients for message transport MUST support the
UDSAgent:1 and UDSController:1 data model profiles.

==== 4.6.1.1 Establishing a UNIX Domain Socket Connection <sec:establishing-a-unix-domain-socket-connection>
...
#strong[#[R\-UDS.2]<r-uds.2>]; \- A USP Endpoint acting as a UNIX domain
socket server MUST bind to a UNIX domain socket and listen for incoming
connections.

#strong[#[R\-UDS.3]<r-uds.3>]; \- A USP Endpoint acting as a UNIX domain
...
==== 4.6.1.2 Retrying a UNIX Domain Socket Connection <sec:retrying-a-unix-domain-socket-connection>
...
#strong[#[R\-UDS.5]<r-uds.5>]; \- When a UNIX domain socket connection
is closed or fails to be established, the USP Endpoint acting as a
client MUST attempt to re\-establish the UNIX domain socket within a
random amount of time between 1 and 5 seconds.

==== 4.6.1.3 Sending a Message over a UNIX Domain Socket <sec:sending-a-message-over-a-unix-domain-socket>
...
#strong[#[R\-UDS.6]<r-uds.6>]; \- A Frame sent across a UNIX domain
socket that is being used as an MTP MUST have a Header field and one or
more TLV fields.

#strong[#[R\-UDS.7]<r-uds.7>]; \- The Header of a Frame sent across a
UNIX domain socket that is being used as an MTP MUST have a
synchronization part and a length part.

#strong[#[R\-UDS.8]<r-uds.8>]; \- The synchronization portion of the
Frame’s Header MUST contain the following 4 bytes: 0x5f 0x55 0x53 0x50
(the hexadecimal version of “\_USP”).

#strong[#[R\-UDS.9]<r-uds.9>]; \- The length portion of the Frame’s
Header MUST contain the length of the remainder of the Frame (i.e.~the
length of the entire Frame excluding the size of the Header) as a 4 byte
unsigned integer in network byte order.

#strong[#[R\-UDS.10]<r-uds.10>]; \- A TLV field contained in a Frame
sent across a UNIX domain socket that is being used as an MTP MUST have
a 1 byte Type.

#strong[#[R\-UDS.11]<r-uds.11>]; \- A TLV field contained in a Frame
sent across a UNIX domain socket that is being used as an MTP MUST have
a 4 byte Length in network byte order.
...
#[
  ...
  #table(
    ...
    [The Handshake contains a UTF\-8 string that represents the Endpoint
    ID of the USP Endpoint sending the message.
    ],
    [`2`
    ],
    [Error
    ],
    [The Error contains a UTF\-8 string that provides the error message
    related to the communications failure.
    ],
    [`3`
    ],
    [USP Record
    ],
    [The USP Record contains the Google Protocol Buffer binary\-encoded
    USP Record being sent between a USP Agent and USP Controller.
    ]
  )
]

#strong[#[R\-UDS.12]<r-uds.12>]; \- A Frame sent across a UNIX domain
socket that is being used as an MTP MUST contain a TLV with Type 1 for
any Handshake negotiation messages

#strong[#[R\-UDS.13]<r-uds.13>]; \- A Frame sent across a UNIX domain
socket that is being used as an MTP MUST contain a TLV with Type 2 for
any Error messages

#strong[#[R\-UDS.14]<r-uds.14>]; \- A Frame sent across a UNIX domain
socket that is being used as an MTP MUST contain a TLV with Type 3 for
any USP Record messages

#strong[#[R\-UDS.15]<r-uds.15>]; \- A Frame sent across a UNIX domain
socket that is being used as an MTP MUST ignore any TLVs that have
unexpected Types

=== 4.6.2 Handshaking with UNIX Domain Sockets <sec:handshaking-with-unix-domain-sockets>
...
#strong[#[R\-UDS.16]<r-uds.16>]; \- A USP Endpoint acting as a UNIX
domain socket client MUST send a Unix domain socket Frame containing a
Type 1 (Handshake) TLV field once it establishes a UNIX domain socket
connection. This message MUST contain the Endpoint ID of the USP
Endpoint sending the message.

#strong[#[R\-UDS.17]<r-uds.17>]; \- A USP Endpoint acting as a UNIX
domain socket server MUST send a Unix domain socket Frame containing a
Type 1 (Handshake) TLV field once it receives a Unix domain socket Frame
...
#strong[#[R\-UDS.18]<r-uds.18>]; \- A USP Endpoint acting as a UNIX
domain socket client MUST terminate the UNIX domain socket connection if
it doesn’t receive a Unix domain socket Frame containing a Type 1
...
#strong[#[R\-UDS.19]<r-uds.19>]; \- A USP Endpoint acting as a UNIX
domain socket client or server MUST ignore an unexpected UNIX domain
socket Frame that contains a Type 2 (Handshake) TLV field.

#strong[#[R\-UDS.20]<r-uds.20>]; \- A USP Endpoint acting as a UNIX
domain socket client or server MUST ignore any UNIX domain socket Frames
that contain a Type 3 (USP Record) TLV field until it has successfully
...
#figure(
  ...
  #bbf-image("mtp/unix-domain-socket/./USP-UDS-Handshake.png")<img:unix-domain-socket-frame-with-handshake-message>
] <fig:unix-domain-socket-frame-with-handshake-message>

==== 4.6.2.1 Handling Failures to Handshake <sec:handling-failures-to-handshake>
...
#strong[#[R\-UDS.21]<r-uds.21>]; \- A USP Endpoint acting as a UNIX
domain socket client or server MUST send a UNIX domain socket Frame
containing a Type 2 (Error) TLV field if it can not process an incoming
UNIX domain socket Frame that contains a Type 1 (Handshake) TLV field.

=== 4.6.3 Sending USP Records across UNIX Domain Sockets <sec:sending-usp-records-across-unix-domain-sockets>
...
#strong[#[R\-UDS.22]<r-uds.22>]; \- A USP Endpoint MUST send Google
Protocol Buffer binary\-encoded USP Records by utilizing a UNIX domain
socket Frame that contains a Type 3 (USP Record) TLV field.
...
#figure(
  ...
  #bbf-image("mtp/unix-domain-socket/./USP-UDS-Record.png")<img:unix-domain-socket-frame-with-usp-record-message>
] <fig:unix-domain-socket-frame-with-usp-record-message>

==== 4.6.3.1 Handling Failures to Deliver USP Records <sec:handling-failures-to-deliver-usp-records-1>
...
#strong[#[R\-UDS.23]<r-uds.23>]; \- A USP Endpoint acting as a UNIX
domain socket client or server MUST send a UNIX domain socket Frame
containing a Type 2 (Error) TLV field and terminate the UNIX Domain
Socket connection, if it receives an incoming UNIX domain socket Frame
...
#strong[#[R\-UDS.24]<r-uds.24>]; \- A USP Endpoint acting as a UNIX
domain socket client or server MUST send a UNIX domain socket Frame
containing a Type 2 (Error) TLV field and terminate the UNIX Domain
Socket connection, if it can not parse an incoming UNIX domain socket
Frame.
...
#strong[#[R\-UDS.25]<r-uds.25>]; \- A USP Endpoint acting as a UNIX
domain socket client or server that receives a UNIX domain socket Frame
...
#strong[#[R\-ENC.0]<r-enc.0>]; \- An implementation using protocol
buffers encoding to encode USP Messages (Requests, Responses, and
Errors) MUST conform to the schema defined in
#link("../specification/usp-msg-1-4.proto")[usp\-msg\-1\-4.proto].

#strong[#[R\-ENC.1]<r-enc.1>]; \- An implementation using protocol
buffers encoding to encode USP Records MUST conform to the schema
defined in
#link("../specification/usp-record-1-4.proto")[usp\-record\-1\-4.proto].
...
#strong[#[R\-ENC.2]<r-enc.2>]; \- If an Endpoint receives a USP payload
...
#strong[#[R\-ENC.3]<r-enc.3>]; \- USP Records and USP Messages that
contain an element of type `oneof` MUST include 1 and only 1 instance of
the element, which MUST contain one of the possible elements.

#strong[#[R\-ENC.4]<r-enc.4>]; \- A USP Record that violates
#link(<r-enc.3>)[R\-ENC.3] MUST be discarded.

#strong[#[R\-ENC.5]<r-enc.5>]; \- A USP Message that violates
#link(<r-enc.3>)[R\-ENC.3] SHOULD return an error of type 7004 (Invalid
Arguments).

== 5.1 Parameter and Argument Value Encoding <sec:parameter-value-encoding>
...
#strong[#[R\-ENC.6]<r-enc.6>]; \- Parameter and argument values MUST be
...
#strong[#[R\-E2E.1]<r-e2e.1>]; \- A receiving USP Endpoint MUST ignore
any Record that does not contain its own Endpoint Identifier as the
`to_id` field of the Record.

#strong[#[R\-E2E.2]<r-e2e.2>]; – A USP Record with record\_type \=
session\_context MUST contain at least one `payload` field, or a
non\-zero `retransmit_id`. (DEPRECATED)

#emph[Note: the R\-E2E.2 requirement was deprecated in USP 1.2, because
sending a Session Context Record without a payload is useful for
restarting a Session Context];

#emph[Note: the requirements below reference Objects and Parameters used
to manage the E2E Session. These are specified in the Device:2 Data
Model #link(<ref-TR-181>)[\[3\]].];

#emph[Note: The USP Record Encapsulation section was moved to
#link(<sec:usp-record-encapsulation>)[USP Record Encapsulation] in USP
1.2.];

== 6.1 Exchange of USP Records within an E2E Session Context <sec:exchange-of-usp-records-within-an-e2e-session-context>
...
=== 6.1.1 Establishing an E2E Session Context <sec:establishing-an-e2e-session-context>
...
#strong[#[R\-E2E.3]<r-e2e.3>]; – Session Context identifiers MUST be
...
#strong[#[R\-E2E.4]<r-e2e.4>]; – When a USP Endpoint receives a USP
Record from another USP Endpoint where there is no established Session
Context, and the USP Record includes a Session Context identifier, and
the USP Endpoint is configured to allow Session Context to be used with
the other Endpoint, the USP Endpoint MUST start a new Session Context
for the remote USP Endpoint, and initialize the `sequence_id` field to
`1`.

#strong[#[R\-E2E.5]<r-e2e.5>]; – At most one (1) Session Context is
established between an Agent and Controller.

#strong[#[R\-E2E.6]<r-e2e.6>]; – When a USP Endpoint receives a USP
Record from a remote USP Endpoint with a different Session Context
identifier than was previously established, the receiving USP Endpoint
MUST start a new Session Context for the remote USP Endpoint, using the
`session_id` from the received USP Record, and initialize the
`sequence_id` field to `1`.

#emph[Note: Implementations need to consider if outstanding USP Messages
that have not been transmitted to the remote USP Endpoint need to be
transmitted within the newly established Session Context.];

#strong[#[R\-E2E.6a]<r-e2e.6a>]; – When an Agent is configured not to
allow Session Context or does not support Session Context and receives a
USP Record initiating Session Context, the Agent MUST reply with a
...
#strong[#[R\-E2E.6b]<r-e2e.6b>]; – If an Agent needs to terminate a
Session Context without terminating an existing MTP connection where
Session Context is being used, it MUST send a Disconnect Record and MUST
include reason\_code and reason indicating Session Context is being
terminated (code 7105 from
#link(<sec:usp-record-errors>)[USP Record Errors]).
...
#strong[#[R\-E2E.7]<r-e2e.7>]; – When a Session Context between a
Controller or Agent expires the Agent MUST initiate a new Session
Context upon the next interaction with the remote USP Endpoint or from a
Session Context request by the remote USP Endpoint.

==== 6.1.1.2 Exhaustion of Sequence Identifiers <sec:exhaustion-of-sequence-identifiers>
...
#strong[#[R\-E2E.8]<r-e2e.8>]; – When a USP Endpoint receives a USP
Record with a value of the `sequence_id` field that is within 10,000 of
the maximum size for the data type of the `sequence_id` field, the USP
Endpoint MUST establish a new Session Context with the remote USP
Endpoint.

#strong[#[R\-E2E.9]<r-e2e.9>]; – When a USP Endpoint transmits a USP
Record with a value of the `sequence_id` field that is within 10,000 of
the maximum size for the data type of the `sequence_id` field, the USP
Endpoint MUST establish a new Session Context with the remote USP
Endpoint upon its next contact with the remote USP Endpoint.

==== 6.1.1.3 Failure Handling in the Session Context <sec:failure-handling-in-the-session-context>
...
#strong[#[R\-E2E.10]<r-e2e.10>]; – When retrying USP Records, the Agent
MUST use the following retry algorithm to manage the retransmission
Session Context establishment procedure:
...
#[
  ...
  #block(
    ...
    #table(
      ...
      [`Device.LocalAgent.Controller.{i}.E2ESession.SessionRetryMinimumWaitInterval`
      ],
      [Interval multiplier
      ],
      [k
      ...
      [`Device.LocalAgent.Controller.{i}.E2ESession.SessionRetryIntervalMultiplier`
      ]
    )
  ]
]
...
#strong[#[R\-E2E.11]<r-e2e.11>]; \- Beginning with the tenth retry
attempt, the Agent MUST choose from the fixed maximum range. The Agent
will continue to retry a failed session establishment until a USP
Message is successfully received by the Agent or until the
SessionExpiration time is reached.

#strong[#[R\-E2E.12]<r-e2e.12>]; – Once a USP Record is successfully
received, the Agent MUST reset the Session Context retry count to zero
for the next Session Context establishment.

#strong[#[R\-E2E.13]<r-e2e.13>]; – If a reboot of the Agent occurs, the
...
#strong[#[R\-E2E.14]<r-e2e.14>]; – When an originating USP Endpoint
...
#strong[#[R\-E2E.15]<r-e2e.15>]; – When an originating USP Endpoint
...
#strong[#[R\-E2E.16]<r-e2e.16>]; – When an originating USP Endpoint
...
#strong[#[R\-E2E.17]<r-e2e.17>]; – When an originating USP Endpoint
...
==== 6.1.2.2 Payload Security within the Session Context <sec:payload-security-within-the-session-context>
...
#strong[#[R\-E2E.18]<r-e2e.18>]; – The originating USP Endpoint MUST use
the same value in the `payload_security` field for all USP Records
...
#figure(
  ...
  #bbf-image("e2e-message-exchange/processing-received-records.png")<img:processing-of-received-usp-records>
] <fig:processing-received-records>
#strong[#[R\-E2E.19]<r-e2e.19>]; – The receiving USP Endpoint MUST
ensure that the value in the `payload_security` field for all USP
Records within a Session Context is the same and fail the USP Record if
the value of the `payload_security` field is different.

#strong[#[R\-E2E.20]<r-e2e.20>]; – Incoming USP Records MUST be
processed per the following rules:
...
===== 6.1.2.3.1 Failure Handling of Received USP Records Within a Session Context <sec:failure-handling-of-received-usp-records-within-a-session-context>
...
#strong[#[R\-E2E.21]<r-e2e.21>]; – When a USP Endpoint that receives a
USP Record within a Session Context that fails to buffer or successfully
...
=== 6.1.3 Guidelines for Handling Session Context Restarts <sec:guidelines-for-handling-session-context-restarts>
...
#strong[#[R\-E2E.22]<r-e2e.22>]; – The receiving USP Endpoint MUST
...
#strong[#[R\-E2E.23]<r-e2e.23>]; – USP Records that do not pass
integrity checks MUST be silently ignored and the receiving USP Endpoint
MUST restart the Session Context.
...
#strong[#[R\-E2E.24]<r-e2e.24>]; – USP Records that pass the integrity
check but have an invalid value in the `session_id` field MUST be
silently ignored and the receiving USP Endpoint MUST restart the Session
Context.
...
#figure(
  ...
  #bbf-image("e2e-message-exchange/segmentation-and-reassembly.png")<img:e2e-segmentation-and-reassembly>
] <fig:segmentation-and-reassembly>
#emph[Note: the 64k size limit is not inherent to the STOMP protocol. It
is merely provided here as an example.];

#emph[Note: for other protocols (e.g., MQTT), the maximum allowed size
by the MTP may also include its own header size, not only the conveyed
payload (e.g., MQTT packet size). In this case, the `MaxUSPRecordSize`
value must be a smaller value than the maximal size of the MTP.];
...
#emph[Note: The maximum allowed USP Record size can be exposed via the
...
#strong[Case 1: payload\_security \= PLAINTEXT, single USP Record];

Conditions:

+ Maximum USP Record size > size of (USP Message + USP Record header)
#bbf-image("e2e-message-exchange/segmentation-case-1.png")

#strong[Case 2: payload\_security \= PLAINTEXT, fragmented across
multiple USP Records];

Conditions:

+ Maximum USP Record size \< size of (USP Message + USP Record header)
#bbf-image("e2e-message-exchange/segmentation-case-2.png")

#strong[Case 3: payload\_security \= TLS12, single TLS record, single
USP Record];
...
#bbf-image("e2e-message-exchange/segmentation-case-3.png")

#strong[Case 4: Payload\_security \= TLS12, all TLS records in a single
USP Record];
...
+ Maximum USP Record size > size of all TLS records + size of USP Record
  header
#bbf-image("e2e-message-exchange/segmentation-case-4.png")

#strong[Case 5: Payload\_security \= TLS12, single TLS record fragmented
across multiple USP Records];
...
#bbf-image("e2e-message-exchange/segmentation-case-5.png")

#strong[Case 6: Payload\_security \= TLS12, multiple TLS records, one
TLS record per USP Record];
...
+ Maximum USP Record size \< size of USP Message + size of TLS record
  header + size of USP Record header
#bbf-image("e2e-message-exchange/segmentation-case-6.png")

#strong[Case 7: Payload\_security \= TLS12, multiple TLS records, some
TLS records fragmented across multiple USP Records];
...
+ Maximum USP Record size \< size of (some TLS records + USP Record
  header)
#bbf-image("e2e-message-exchange/segmentation-case-7.png")

=== 6.1.5 Handling Duplicate USP Records <sec:handling-duplicate-usp-records>
...
#strong[#[R\-E2E.25]<r-e2e.25>]; \- When exchanging USP Records with an
E2E Session Context, if a target USP Endpoint receives a USP Record with
...
== 6.2 Exchange of USP Records without an E2E Session Context <sec:exchange-of-usp-records-without-an-e2e-session-context>
...
#strong[#[R\-E2E.26]<r-e2e.26>]; \- A `record_type` of
`no_session_context` MUST be used for exchange of USP Records without an
E2E Session Context. A non\-zero `payload` MUST be included.

=== 6.2.1 Failure Handling of Received USP Records Without a Session Context <sec:failure-handling-of-received-usp-records-without-a-session-context>

When a receiving USP Endpoint fails to either buffer or successfully
process a USP Record, the receiving USP Endpoint reports a failure.

#strong[#[R\-E2E.27]<r-e2e.27>]; (DEPRECATED) \- When a USP Endpoint
that receives a USP Record without a Session Context that fails to
buffer or successfully process (e.g., decode, decrypt, retransmit) the
USP Endpoint SHOULD send a `DisconnectRecord` (as described in
#link(<r-mtp.7>)[R\-MTP.7] for Agents).

#emph[Note: Requirement #link(<r-e2e.27>)[R\-E2E.27] was removed in USP
1.3, replaced by the behavior defined in #link(<r-mtp.5>)[R\-MTP.5]];
...
== 6.3 Validating the Integrity of the USP Record <sec:validating-the-integrity-of-the-usp-record>
...
#strong[#[R\-E2E.28]<r-e2e.28>]; \- When a USP Record is received or
...
#strong[#[R\-E2E.29]<r-e2e.29>]; – Unless protected by the underlying
MTP, when a USP Endpoint transmits a USP Record, the USP Endpoint MUST
protect the integrity of the non\-payload portion of the USP Record.

#strong[#[R\-E2E.30]<r-e2e.30>]; – When a USP Endpoint receives a USP
...
=== 6.3.1 Using the Signature Method to Validate the Integrity of USP Records <sec:using-the-signature-method-to-validate-the-integrity-of-usp-records>
...
`ECDSA_P256_SHA256_ASN1` in this specification.

#strong[#[R\-E2E.31]<r-e2e.31>]; – When using the signature method to
...
=== 6.3.2 Using TLS to Validate the Integrity of USP Records <sec:using-tls-to-validate-the-integrity-of-usp-records>
...
#strong[#[R\-E2E.32]<r-e2e.32-1>.1]; – When generating or validating the
MAC or signature, all non\-payload fields MUST be appended as byte
arrays and fed into the MAC or signature generation function with the
...
#strong[#[R\-E2E.33]<r-e2e.33>]; – If using the TLS MAC method to
protect the integrity of a USP Record, and a USP Endpoint receives a USP
Record, the USP Endpoint MUST verify the MAC using the SHA\-256 HMAC
algorithm for the non\-payload portion of the USP Record.

#strong[#[R\-E2E.34]<r-e2e.34>]; – If using the TLS MAC method to
protect the integrity of a USP Record, when generating or validating the
MAC of the USP Record, the sequence of the non\-payload fields MUST use
the field identifier of the USP Record’s protobuf specification
...
protect the integrity of a USP Record, when generating or validating the
MAC of the USP Record, the USP Endpoint MUST use a label value of
"`EXPORTER-BBF-USP-Record`" and a zero length context.

#strong[#[R\-E2E.37]<r-e2e.37>]; – If using the TLS MAC method to
protect the integrity of a USP Record, when generating or validating the
MAC of the USP Record, the USP Endpoint MUST generate 64 octets of
keying material.
...
protect the integrity of a USP Record, when generating the MAC of the
USP Record, the USP Endpoint MUST use the first 32 octets of the keying
...
#strong[#[R\-E2E.40]<r-e2e.40>]; – When using TLS to protect USP
payloads in USP Records, USP Endpoints MUST implement TLS 1.2 or later
(with backward compatibility to TLS 1.2) with the
`ECDHE-ECDSA-AES128-GCM-SHA256` cipher for TLS 1.2 and the
`TLS-AES128-GCM-SHA256` cipher for TLS 1.3.

#strong[#[R\-E2E.40a]<r-e2e.40a>]; \- When using TLS to protect USP
payloads in USP Records, USP Endpoints MUST use ECDHE for key exchange
and MUST support the named group `secp256r1` (NIST P\-256 curve) for use
in ECDHE.

#strong[#[R\-E2E.40b]<r-e2e.40b>]; \- When using TLS to protect USP
payloads in USP Records, USP Endpoints MUST use the ECDSA signature
scheme with the NIST P\-256 curve and SHA\-256.

#emph[Note: The requirements listed above require a USP Endpoint to use
X.509 certificates with an Elliptic\-curve public key compatible with
...
#figure(
  ...
  #bbf-image("e2e-message-exchange/tls-session-handshake.png")<img:tls-session-handshake>
] <fig:handshake>
#strong[#[R\-E2E.41]<r-e2e.41>]; – USP Endpoints that specify `TLS12` in
the `payload_security` field MUST exchange USP Records within an E2E
Session Context.
...
#strong[#[R\-E2E.42]<r-e2e.42>]; – USP Endpoints MUST NOT accept
requests for TLS renegotiation when used for E2E Message exchange. USP
Endpoints MAY send a TLS `no_renegotiation` alert in response to a
...
#strong[#[R\-E2E.43]<r-e2e.43>]; – USP Endpoints MUST be mutually
...
#emph[Note: This version of the specification defines its Messages in
#link(<sec:encoding>)[Protocol Buffers v3]. This part of the
specification may change to a more generic description (normative and
non\-normative) if further encodings are specified in future versions.];
...
== 7.2 Requests, Responses and Errors <sec:requests-responses-and-errors>
...
#strong[#[R\-MSG.0]<r-msg.0>]; \- The target USP Endpoint MUST respond
to a request Message from the source USP Endpoint with either a response
...
#strong[#[R\-MSG.0a]<r-msg.0a>]; \- The associated response or error
Message MUST be sent through the same type of USP Record (i.e.~A
`record_type` of
...
#strong[#[R\-MSG.1]<r-msg.1>]; \- The target USP Endpoint MUST ignore or
send an Error Message in response to Messages it does not understand.

#emph[Note: Requirement R\-MSG.2 was removed in USP 1.2, because it did
not align with the concept of brokered MTPs and long\-lived connections
in general.];

#strong[#[R\-MSG.3]<r-msg.3>]; \- In any USP Message originating from an
...
#strong[#[R\-MSG.4]<r-msg.4>]; \- If a target USP Endpoint receives a
...
#strong[#[R\-MSG.4a]<r-msg.4a>]; \- Unless otherwise specified, if a
Request contains a Search Path, the associated Response MUST result in a
...
#emph[Note: this is not meant to imply that all request\/response
...
#figure(
  ...
  #bbf-image("messages/successful_response.png")<img:a-successful-requestresponse-sequence>
...
#figure(
  ...
  #bbf-image("messages/error_response.png")<img:a-failed-requestresponse-sequence>
...
#strong[#[R\-MSG.5]<r-msg.5>]; \- A Message MUST conform to the schemas
defined in
#link("../specification/usp-msg-1-4.proto")[usp\-msg\-1\-4.proto].

#emph[See the section on #link(<sec:usp-record-encapsulation>)[USP
Record Encapsulation] for information about Protocol Buffers default
behavior and required fields.];
...
type according to Protocol Buffers #link(<ref-PROTOBUF>)[\[4\]],
...
#strong[#[R\-MSG.6]<r-msg.6>]; \- A Message MUST contain exactly one
header field.
...
#strong[#[R\-MSG.7]<r-msg.7>]; \- A Message MUST contain exactly one
body field.
...
#strong[#[R\-MSG.8]<r-msg.8>]; \- The msg\_id field MUST be present in
every Header.

#strong[#[R\-MSG.9]<r-msg.9>]; \- The msg\_id field in the Message
Header for a Response or Error that is associated with a Request MUST
contain the Message ID of the associated request. If the msg\_id field
in the Response or Error does not contain the Message ID of the
associated Request, the response or error MUST be ignored.
...
```
    ...
    DEREGISTER_RESP (22)
```
#strong[#[R\-MSG.10]<r-msg.10>]; \- The `msg_type` field MUST be present
in every Header. Though required, it is meant for information only. In
the event this field differs from the `req_type` or `resp_type` in the
...
== 7.4 Creating, Updating, and Deleting Objects <sec:creating-updating-and-deleting-objects>
...
=== 7.4.1 Selecting Objects and Parameters <sec:selecting-objects-and-parameters>
...
#strong[#[R\-KEY.1]<r-key.1>]; \- Non\-functional Unique Keys (as
defined in TR\-106 #link(<ref-TR-106>)[\[2\]]) MUST NOT change in the
Agent’s Instantiated Data Model after creation, as defined in
#link(<r-add.5>)[R\-ADD.5].

#strong[#[R\-KEY.2]<r-key.2>]; \- Functional Unique Keys (as defined in
...
=== 7.4.3 writeOnceReadOnly Parameter Access <sec:writeoncereadonly-parameter-access>

There are several parameter access types defined in
#link(<ref-TR-106>)[\[2\]] that specify what a Controller is allowed to
do with a particular Parameter as defined in #link(<ref-TR-181>)[\[3\]].
Of these, `writeOnceReadOnly` affects the response of an Agent in
specific ways. A `writeOnceReadOnly` Parameter can be set only once by a
Controller, and then becomes read\-only. In some cases, the value is
assigned by the Agent upon Object creation if not specified by the
Controller. The expected behavior is otherwise as follows:

- When a `writeOnceReadyOnly` Parameter is set upon Object creation via
  the `Add` Message, the Controller making the Add Request is considered
  to have set the Parameter.
- When a `writeOnceReadOnly` Parameter is updated via a `Set` Message,
  an Agent will reject the update if the Parameter has already been set.
- `writeOnceReadyOnly` Parameters are reported as `PARAM_READ_WRITE (1)`
  in a `GetSupportedDM` Response. Controllers should rely on knowledge
  of the Agent’s data model through other means (i.e., officially
  published data models) to determine that a parameter is
  `writeOnceReadOnly`.
...
#emph[Note: It is up to the individual implementation whether to abort
and return an Error Message after the first error, or provide
information about multiple failures.];
...
#[
  ...
  #table(
    ...
    fill: bbf-table-fill.with(columns: 7, header-rows: 1),
    table.header(
    [`allow_partial`
    ],
    [Required Parameters
    ...
    [Other Parameter Failed
    ],
    [Response\/Error
    ],
    [Oper\_status of Object
    ],
    [Contains param\_errs
    ]),
    [`true`\/`false`
    ],
    [No
...
==== 7.4.4.1 Search Paths and allow\_partial in Set <sec:search-paths-in-set>
...
==== 7.4.5.2 Add Request Fields <sec:add-request-fields>
...
#strong[#[R\-ADD.0]<r-add.0>]; \- If the `allow_partial` field is set to
...
#strong[#[R\-ADD.1]<r-add.1>]; \- If the `allow_partial` field is set to
...
===== 7.4.5.2.1 CreateObject Fields <sec:createobject-fields>
...
#strong[#[R\-ADD.2]<r-add.2>]; \- The `obj_path` field in the
`CreateObject` Message of an Add Request MUST specify or match exactly
one Object Path. (DEPRECATED)

#emph[Note: The R\-ADD.2 requirement was deprecated in USP 1.3 because
...
====== 7.4.5.2.1.1 CreateParamSetting Fields <sec:createparamsetting-fields>
...
- #emph[If the Add Message omits Parameters for which the Controller
  does not have write permission, those parameters will be set to their
  default (if any) by the Agent, and the Add Message succeeds.];
- #emph[If the Add Message includes Parameters for which the Controller
  does not have write permission, the Message proceeds in accordance
  with the rules for allow\_partial and required parameters.];
`string value`
...
#emph[Note: Any Unique Key Parameter contained in the Add Message will
...
#strong[#[R\-ADD.3]<r-add.3>]; \- If the `required` field is set to
true, a failure to update this Parameter MUST result in a failure to
create the Object.

==== 7.4.5.3 Add Response Fields <sec:add-response-fields>
...
===== 7.4.5.3.1 CreatedObjectResult Fields <sec:createdobjectresult-fields>
...
====== 7.4.5.3.1.1 OperationStatus Fields <sec:operationstatus-fields>
...
====== 7.4.5.3.1.2 OperationFailure Fields <sec:operationfailure-fields>
...
====== 7.4.5.3.1.3 Operation Success Fields <sec:operation-success-fields>
...
#strong[#[R\-ADD.4]<r-add.4>]; \- If any of the Parameters and values
...
#strong[#[R\-ADD.5]<r-add.5>]; \- If the Controller did not include some
or all of the Unique Key Parameters that are supported by the Agent in
the `param_settings` field, the Agent MUST assign values to these
Parameters and return them in the `unique_keys` field.

#strong[#[R\-ADD.6]<r-add.6>]; \- If the Controller does not have Read
permission on any of the Parameters returned in `unique_keys`, these
Parameters MUST NOT be returned in this field.

====== 7.4.5.3.1.4 ParameterError Fields <sec:parametererror-fields>
...
==== 7.4.6.2 Set Request Fields <sec:set-request-fields>
...
#strong[#[R\-SET.0]<r-set.0>]; \- If the `allow_partial` field is set to
true, and no other exceptions are encountered, the Agent treats each
...
#emph[Note: This may cause some counterintuitive behavior if there are
no required Parameters to be updated. The Set Request can still result
in a Set Response (rather than an Error Message) if `allow_partial` is
set to true.];

#strong[#[R\-SET.1]<r-set.1>]; \- If the `allow_partial` field is set to
false, and no other exceptions are encountered, the Agent treats each
...
===== 7.4.6.2.1 UpdateObject Fields <sec:updateobject-fields>
...
====== 7.4.6.2.1.1 UpdateParamSetting Fields <sec:updateparamsetting-fields>
...
#strong[#[R\-SET.2]<r-set.2>]; \- If the `required` field is set to
`true`, a failure to update this Parameter MUST result in a failure to
update the Object (see
#link(<sec:using-allow-partial-and-required-parameters>)[Using Allow Partial and Required Parameters]).
...
#strong[#[R\-SET.2b]<r-set.2b>]; \- The Agent MAY terminate processing a
Set Request with an `obj_path` field in the `UpdateObject` message that
contains a Search Path matching more than one object after encountering
any number of errors.

==== 7.4.6.3 Set Response <sec:set-response>
...
===== 7.4.6.3.1 UpdatedObjectResult Fields <sec:updatedobjectresult-fields>
...
====== 7.4.6.3.1.1 OperationStatus Fields <sec:operationstatus-fields-1>
...
====== 7.4.6.3.1.2 OperationFailure Fields <sec:operationfailure-fields-1>
...
====== 7.4.6.3.1.3 UpdatedInstanceFailure Fields <sec:updatedinstancefailure-fields>
...
====== 7.4.6.3.1.4 ParameterError Fields <sec:parametererror-fields-1>
...
====== 7.4.6.3.1.5 OperationSuccess Fields <sec:operationsuccess-fields>

`repeated UpdatedInstanceResult updated_inst_results`

This field contains a repeated set of `UpdatedInstanceResult` messages.

====== 7.4.6.3.1.6 UpdatedInstanceResult Fields <sec:updatedinstanceresult-fields>
...
#strong[#[R\-SET.3]<r-set.3>]; \- If the Controller does not have Read
permission on any of the Parameters specified in `updated_params`, these
Parameters MUST NOT be returned in this field.

#emph[Note: If the Set Request configured a Parameter to the same value
it already had, this Parameter is still returned in the
`updated_params`.];

====== 7.4.6.3.1.7 ParameterError Fields <sec:parametererror-fields-2>
...
==== 7.4.6.4 Set Message Supported Error Codes <sec:set-message-supported-error-codes>

Appropriate error codes for the Set Message include `7000-7016`, `7020`,
`7021`, `7026`, and `7800-7999`.

=== 7.4.7 The Delete Message <sec:delete>

The Delete Message is used to remove Instances of Multi\-Instance
Objects in the Agent’s Instantiated Data Model.

==== 7.4.7.1 Delete Example <sec:delete-example>
...
==== 7.4.7.2 Delete Request Fields <sec:delete-request-fields>
...
#strong[#[R\-DEL.0]<r-del.0>]; \- If the `allow_partial` field is set to
true, and no other exceptions are encountered, the Agent treats each
...
==== 7.4.7.3 Delete Response Fields <sec:delete-response-fields>

`repeated DeletedObjectResult deleted_obj_results`

This field contains a repeated set of `DeletedObjectResult` messages.

===== 7.4.7.3.1 DeletedObjectResult Fields <sec:deletedobjectresult-fields>
...
====== 7.4.7.3.1.1 OperationStatus Fields <sec:operationstatus-fields-2>
...
====== 7.4.7.3.1.2 OperationFailure Fields <sec:operationfailure-fields-2>
...
====== 7.4.7.3.1.3 OperationSuccess Fields <sec:operationsuccess-fields-1>

`repeated string affected_paths`

This field returns a repeated set of Path Names to Object Instances.

#strong[#[R\-DEL.2]<r-del.2>]; \- If the Controller does not have Read
permission on any of the Objects specified in `affected_paths`, these
Objects MUST NOT be returned in this field.

#strong[#[R\-DEL.2a]<r-del.2a>]; \- If the requested\_path was valid
(i.e., properly formatted and in the Agent’s supported data model) but
did not resolve to any Objects in the Agent’s instantiated data model,
the Agent MUST return an OperationSuccess for this requested\_path, and
include an empty set for affected\_path.
...
#strong[#[R\-DEL.3]<r-del.3>]; \- This set MUST include one
...
#strong[#[R\-DEL.4]<r-del.4>]; \- If the Controller does not have Read
permission on any of the Objects specified in `unaffected_paths`, these
Objects MUST NOT be returned in this field.

====== 7.4.7.3.1.4 UnaffectedPathError Fields <sec:unaffectedpatherror-fields>
...
==== 7.4.7.4 Delete Message Supported Error Codes <sec:delete-message-supported-error-codes>

Appropriate error codes for the Delete Message include `7000-7008`,
`7015`, `7016`, `7018`, `7024`, `7026` and `7800-7999`.

== 7.5 Reading an Agent’s State and Capabilities <sec:reading-an-agents-state-and-capabilities>
...
#emph[Note: Those familiar with Broadband Forum TR\-069
#link(<ref-TR-069>)[\[1\]] will recognize this behavior as the
...
matched by the expression(s) supplied in the request.];

#emph[Note: Each Search Path is intended to be evaluated separately, and
the results from a given Search Path are returned in a field dedicated
to that Path Name. As such, it is possible that the same information may
be returned from more than one Search Path. This is intended, and the
Agent should treat each Search Path atomically.];
...
Parameter Paths are Relative Paths to the `resolved_path`. #emph[Note:
...
each Sub\-Object is returned in its own `resolved_path`.];
...
#emph[Note: The `max_depth` field was introduced in USP 1.2. If this
field is not present in a Get request or has a value of `0`, the Agent
returns the complete tree of all Objects and Sub\-Objects of all the
Path Names mentioned in `param_paths`. This is the same as the behavior
specified for prior USP versions. An Agent implementing a prior version
of the USP specification will ignore the field and behave as if the
`max_depth` field was set to `0`.];
...
#[
  ...
  #table(
    ...
    [All the Parameters of `Device.DeviceInfo.` and all of its
    Sub\-Objects (like `Device.DeviceInfo.TemperatureStatus.` and
    ...
    [All the Parameters of `Device.DeviceInfo.` and their values only
    ],
    [`2`
    ],
    [`Device.DeviceInfo.`
    ],
    [All the Parameters of `Device.DeviceInfo.` and its first level
    Sub\-Objects(like `Device.DeviceInfo.TemperatureStatus.`) along with
    their values
    ],
    [`3`
    ],
    ...
    [All the Parameters of `Device.DeviceInfo.` and its first and second
    level Sub\-Objects (like `Device.DeviceInfo.TemperatureStatus.` and
    `Device.DeviceInfo.TemperatureStatus.TemperatureSensor.{i}.`) along
    with their values
    ]
  )
...
#strong[#[R\-GET.5]<r-get.5>]; \- If the `max_depth` field is present
and contains a value other than 0, then the Agent MUST limit the tree
depth of the resolved Sub\-Objects included in the
`resolved_path_results` field of the Response to the specified value.
...
===== 7.5.1.3.1 RequestedPathResult Field <sec:requestedpathresult-field>
...
#strong[#[R\-GET.0]<r-get.0>]; \- If `requested_path` contains a Path
Name (that is not a Search Path) that does not match any Object or
Parameter in the Agent’s Instantiated Data Model, or `requested_path`
contains a Search Path that does not match any Object or Parameter in
the Agent’s Supported Data Model, the Agent MUST use the
`7026 - Invalid Path` error in this `RequestedPathResult`.

#strong[#[R\-GET.1]<r-get.1>]; \- If the Controller making the Request
does not have Read permission on an Object or Parameter matched through
the `requested_path` field, the Object or Parameter MUST be treated as
if it is not present in the Agent’s Supported data model.

#emph[Note: Requiring Object Read permission is intended to act as a
security feature. If a Controller does not have permission to obtain the
names of the data model elements of an object via a `GetSupportedDM`
request, then the Controller should not be able to discover these by
probing with `Get` requests.];
...
#strong[#[R\-GET.1a]<r-get.1a>]; \- If the `requested_path` is a valid
...
#strong[#[R\-GET.1b]<r-get.1b>]; \- If the `requested_path` is a valid
...
====== 7.5.1.3.1.1 ResolvedPathResult Fields <sec:resolvedpathresult-fields>
...
#strong[#[R\-GET.2]<r-get.2>]; \- If the `requested_path` included a
Path Name to a Parameter, the `resolved_path` MUST contain only the Path
Name to the parent Object or Object Instance of that Parameter.
...
#strong[#[R\-GET.3]<r-get.3>]; \- If the `requested_path` included a
Path Name to a Parameter, `result_params` MUST contain only the
Parameter included in that Path Name.

#strong[#[R\-GET.4]<r-get.4>]; \- If the Controller has Read permission
on the Object or Object Instance being returned in the `resolved_path`,
but does not have Read permission on any of the Parameters specified in
`result_params`, these Parameters MUST NOT be returned in this field.
This MAY result in this field being empty.

==== 7.5.1.4 Get Message Supported Error Codes <sec:get-message-supported-error-codes>
...
#emph[possibly]; any Multi\-Instance Sub\-Objects that exist as well as
...
means that the Response returns data #emph[only]; for the Object
...
For example, if a Controller wanted to know #emph[only]; the current
instances of Wi\-Fi SSID Objects that exist on an Agent (that has 2
SSIDs), it would send a GetInstances Request as:
...
==== 7.5.2.2 GetInstances Request Fields <sec:getinstances-request-fields>
...
==== 7.5.2.3 GetInstances Response Fields <sec:getinstances-response-fields>
...
of #emph[all]; of the Objects matched by `requested_path` that exists in
the Agent’s Instantiated Data Model.
...
#strong[#[R\-GIN.1]<r-gin.1>]; \- If the Controller does not have Read
...
==== 7.5.3.2 GetSupportedDM Request Fields <sec:getsupporteddm-request-fields>

`repeated string obj_paths`

This field contains a repeated set of Path Names to Objects, Commands,
Events, or Parameters in the Agent’s Supported or Instantiated Data
...
This field, if `true`, indicates that, in the `supported_objs`, the
Agent should include a `unique_key_sets` field containing Parameters
which uniquely identify an instance of the reported Object(s).
...
===== 7.5.3.3.1 RequestedObjectResult Fields <sec:requestedobjectresult-fields>
...
#strong[#[R\-GSP.0]<r-gsp.0>]; \- If the Controller making the Request
does not have Read permission on an Object or Parameter matched through
the `requested_path` field, the Object or Parameter MUST be treated as
if it is not present in the Agent’s Supported Data Model.
...
====== 7.5.3.3.1.1 SupportedObjectResult Fields <sec:supportedobjectresult-fields>
...
Command supported by the reported Object. If there are no Commands
supported by the Object, this should be an empty list.

`repeated SupportedEventResult supported_events`

The field contains a message of type `SupportedEventResult` for each
Event supported by the reported Object. If there are no Events supported
by the Object, this should be an empty list.

`repeated SupportedUniqueKeySet unique_key_sets`

The field contains a message of type `SupportedUniqueKeySet` for each
UniqueKeySet supported by the reported Object. If the Object has no
unique keys (for example a single instance object), this should be an
empty list.
...
#emph[Note: The `divergent_paths` field was added in USP 1.2. An Agent
that supports versions before USP 1.2 would not know to send the
`divergent_paths` field and thus an empty list will be seen by the
Controller.];

====== 7.5.3.3.1.2 SupportedParamResult Fields <sec:supportedparamresult-fields>
...
based on rules defined in the Agent’s Access Control List. If the Data
Model indicates that a Parameter is writeOnceReadOnly, the
GetSupportedDM returns PARAM\_READ\_WRITE. It is an enumeration of:
...
#emph[primitive (or base) data type]; of this Parameter in the Agent’s
...
```
#emph[Note: The value\_type field was added in USP 1.2, and the
...
#emph[Note: This refers to the data type of the Parameter as implemented
on the device, even though the value itself is transmitted as a Protocol
Buffers string.];
...
```
#emph[Note: The value\_change field was added in USP 1.2, and the
...
(VALUE\_CHANGE\_UNKNOWN) will be seen by the Controller.];

====== 7.5.3.3.1.3 SupportedCommandResult Fields <sec:supportedcommandresult-fields>
...
#emph[Note: This field only contains the Path Name of the supported
...
```
    ...
    CMD_ASYNC (2)
```
#emph[Note: The command\_type field was added in USP 1.2, and the
...
====== 7.5.3.3.1.5 SupportedUniqueKeySet <sec:supporteduniquekeyset>

`repeated string key_names`

This field contains a repeated set of relative parameters, whose values
together uniquely identify an instance of the object in the instantiated
data model.
...
#emph[Note: when using error `7026` (Invalid path), it is important to
note that in the context of GetSupportedDM this applies to the Agent’s
...
==== 7.5.4.1 GetSupportedProtocol Request Fields <sec:getsupportedprotocol-request-fields>
...
==== 7.5.4.2 GetSupportedProtocolResponse Fields <sec:getsupportedprotocolresponse-fields>
...
#strong[#[R\-REG.0]<r-reg.0>]; \- If the `allow_partial` field is set to
...
#strong[#[R\-REG.1]<r-reg.1>]; \- If the `allow_partial` field is set to
...
#strong[#[R\-REG.2]<r-reg.2>]; \- The path field MUST contain an Object
Path, Command Path, Event Path, or Parameter Path without any instance
numbers. This path MUST NOT not use the Supported Data Model notation
(with {i}), meaning that it is not allowed to register a sub\-object to
...
==== 7.5.5.4 RegisteredPathResult Fields <sec:registeredpathresult-fields>
...
==== 7.5.5.5 Register Message Supported Error Codes <sec:register-message-supported-error-codes>
...
#emph[Note: A Deregister Request does not contain a boolean
`allow_partial`, but the Controller will handle each path in the
Deregister Request individually. In other words, `allow_partial` is
implicitly set to `true` during the deregistration. The USP Controller
will provide information about the success or failure to deregister each
requested path in the Deregister Response message.];
...
#strong[#[R\-DEREG.1]<r-dereg.1>]; \- A USP Agent MUST #emph[only];
deregister Service Elements that it registered with a previous Register
message.

#strong[#[R\-DEREG.2]<r-dereg.2>]; \- An empty path field MUST be
interpreted to deregister all Service Elements belonging to the USP
Agent.

#strong[#[R\-DEREG.3]<r-dereg.3>]; \- A USP Agent MAY deregister one or
more Service Elements with one Deregister Request message containing
multiple path fields.

#emph[Note: The path field contains an Object Path, Command Path, Event
Path, or Parameter Path without any instance numbers. This path doesn’t
contain any sub\-objects to a multi\-instance object.];
...
#strong[#[R\-DEREG.4]<r-dereg.4>]; \- A USP Controller MUST always
respond with a Deregister Response message to a Deregister Request. USP
Error messages are not used.

==== 7.5.6.4 DeregisteredPathResult Fields <sec:deregisteredpathresult-fields>
...
==== 7.5.6.5 Deregister Message Supported Error Codes <sec:deregister-message-supported-error-codes>

Appropriate error codes for the Deregister Message include 7000\-7008,
7016, 7030 and 7800\-7999.

== 7.6 Notifications and Subscription Mechanism <sec:notifications-and-subscriptions>
...
#strong[#[R\-NOT.0]<r-not.0>]; \- The Agent and Controller MUST follow
the normative requirements defined in the
`Device.LocalAgent.Subscription.{i}.` Object specified in the Device:2
Data Model #link(<ref-TR-181>)[\[3\]].

#strong[#[R\-NOT.0a]<r-not.0a>]; \- When considering the time needed to
make a state change and trigger a Notification, an implementation SHOULD
make changes to its state and initiate a Notification with a window no
...
=== 7.6.2 Responses to Notifications and Notification Retry <sec:responses-and-retry>
...
#strong[#[R\-NOT.1]<r-not.1>]; \- When retrying notifications, the Agent
...
#strong[#[R\-NOT.2]<r-not.2>]; \- Beginning with the tenth retry
attempt, the Agent MUST choose from the fixed maximum range. The Agent
will continue to retry a failed notification until it is successfully
delivered or until the `NotifExpiration` time is reached.

#strong[#[R\-NOT.3]<r-not.3>]; \- Once a notification is successfully
delivered, the Agent MUST reset the retry count to zero for the next
notification Message.

#strong[#[R\-NOT.4]<r-not.4>]; \- If a reboot of the Agent occurs, the
Agent MUST reset the retry count to zero for the next notification
Message.
...
==== 7.6.3.2 ObjectCreation and ObjectDeletion <sec:objectcreation-and-objectdeletion>
...
An `OnBoardRequest` notification is used by the Agent to initiate the
request in order to communicate with a Controller that can provide
on\-boarding procedures and communicate with that Controller (likely for
the first time).

#strong[#[R\-NOT.5]<r-not.5>]; \- An Agent MUST send an `OnBoardRequest`
...
+ When the `SendOnBoardRequest()` command is executed. This sends the
  ...
  Device:2 Data Model #link(<ref-TR-181>)[\[3\]]. This requirement
  applies only to those Controller table instances that have their
  `Enabled` Parameter set to `true`. When the implementation supports
  the `Device.LocalAgent.Controller.{i}.OnBoardingComplete` parameter,
  executing this command will set the parameter value to `false`.
+ When instructed to do so by internal application policy (for example,
  when using DHCP discovery defined above).
+ When the implementation supports the
  `Device.LocalAgent.Controller.{i}.OnBoardingComplete` parameter and
  its value is set to false as this parameter signifies the Controller
  requires an `OnBoardRequest`. This parameter SHOULD be set to true by
  the Agent or Controller when the onboarding has been completed.
The value of the `OnBoardingRestartTime` parameter, if supported,
describes how long the USP Agent must wait to send another
`OnBoardRequest` when `OnBoardingComplete` remains `false`. This timer
starts as soon as the Agent receives the `NotifyResponse` message for
the original `OnBoardRequest`, because the Notification Retry mechanism
needs to be completed first.

#emph[Note: as defined in the Subscription table, OnBoardRequest is not
included as one of the enumerated types of a Subscription, i.e., it is
not intended to be the subject of a Subscription. However it is
important that the OnBoardRequest reaches its destination so it MUST be
sent with `send_resp=true`. In other words, it behaves as a subscription
with `NotifRetry` set to true. Because the Controller might be
unreachable for a long time, the notification MUST not expire, so it
MUST behave as a subscripton with `NotifExpiration` set to 0.];

#strong[#[R\-NOT.6]<r-not.6>]; The OnBoardRequest MUST be sent with
send\_resp\=true and MUST follow the Retry logic defined above.

#strong[#[R\-NOT.6a]<r-not.6a>]; The OnBoardRequest MUST be retried
until the Controller confirms it has received it with a Notify Response
message.
...
#strong[#[R\-NOT.7]<r-not.7>]; \- The `subscription_id` field MUST
contain the Subscription ID of the Subscription Object that triggered
this notification. If no subscription\_id is available (for example, for
...
#strong[#[R\-NOT.8]<r-not.8>]; \- When `send_resp` is set to false, the
...
#strong[#[R\-NOT.9]<r-not.9>]; \-The Agent SHOULD ignore the
subscription\_id field.

#emph[Note: The requirement in the previous versions of the
specification requiring the Agent to check this subscription\_id field
has been deprecated. However for backward compatibility the Controller
is still required to send the matching subscription\_id.];

#strong[#[R\-NOT.10]<r-not.10>]; \- The Controller MUST populate the
...
#figure(
  ...
  #bbf-image("messages/synchronous_operation.png")<img:operate-message-flow-for-synchronous-operations>
...
#strong[#[R\-OPR.0]<r-opr.0>]; \- When an Agent receives an Operate
Request that addresses an asynchronous operation, it MUST create a
Request Object in the Request table of its Instantiated Data Model (see
the Device:2 Data Model #link(<ref-TR-181>)[\[3\]]). When the Operation
is complete (either success or failure), it MUST remove this Object from
the Request table.
...
#figure(
  ...
  #bbf-image("messages/asynchronous_operation.png")<img:operate-message-flow-for-asynchronous-operations>
] <fig:operate-message-flow-for-asynchronous-operations>

==== 7.7.2.1 Persistence of Asynchronous Operations <sec:persistence-of-asynchronous-operations>
...
=== 7.7.3 Operate Requests on Multiple Objects <sec:operate-requests-on-multiple-objects>
...
#emph[Note: This specification does not make any requirement on the
order in which commands on multiple objects selected with a Path Name
expression are executed.];

#strong[#[R\-OPR.1]<r-opr.1>]; \- When processing Operate Requests on
...
#strong[#[R\-OPR.2]<r-opr.2>]; \- For asynchronous operations the Agent
MUST create a separate Request Object for each Object and associated
operation matched in the command field.

=== 7.7.4 Event Notifications for Operations <sec:event-notifications-for-operations>
...
#strong[#[R\-OPR.3]<r-opr.3>]; \- When handling concurrently invoked
...
In this example of a synchronous command, the Controller requests that
the Agent initiate the SendOnBoardRequest() operation defined in the
...
```
In this example of an asynchronous command, the Controller requests that
the Agent initiate the SelfDiagnostics() operation defined in the
`Device.` object.
...
#strong[#[R\-OPR.4]<r-opr.4>]; \- When `send_resp` is set to `false`,
the target Endpoint SHOULD NOT send an `OperateResp` Message to the
source Endpoint. If an error occurs during the processing of an
`Operate` Message, the target Endpoint MAY send an `OperateResp` Message
to the source Endpoint containing relevant `cmd_failure` elements. If a
response is still sent, the responding Endpoint MUST expect that any
such response will be ignored.

#emph[Note: The requirement in the previous versions of the
specification also discouraged the sending of an `Error` Message,
however the Controller issuing the `Operate` might want to learn about
and handle errors occurring during the processing of the `Operate`
request but still ignore execution results.];
...
#strong[#[R\-OPR.5]<r-opr.5>]; \- A `Command` can have mandatory
...
in its `OperateResp`, include a `cmd_failure` element containg an
`err_code` of type `7027` Invalid Command Arguments and stop processing
the command.

#strong[#[R\-OPR.6]<r-opr.6>]; \- When an unrecognized Input argument is
included in the `input_args` field, the Agent MUST ignore the Input
argument and continue processing the `Operate` Message.

#strong[#[R\-OPR.7]<r-opr.7>]; \- When a non\-mandatory Input argument
is omitted from the `input_args` field, the Agent MUST use a default
value for the missing Input argument and continue processing the
`Operate` Message.
...
==== 7.7.7.3 Operate Message Error Codes <sec:operate-message-error-codes>
...
#[
  ...
  #table(
    ...
    [This error indicates a general failure that is described in an
    err\_msg field.
    ],
    [`7001`
    ],
    ...
    [This error indicates that the attempted message was not understood
    ...
    [This error indicates that the target Endpoint cannot or will not
    process the message or operation.
    ],
    [`7003`
    ],
    [Internal error
    ...
    [This error indicates that the message or operation failed due to
    internal hardware or software reasons.
    ],
    [`7004`
    ],
    [Invalid arguments
    ...
    [This error indicates that the message or operation failed due to
    invalid values in the USP message.
    ],
    [`7005`
    ],
    [Resources exceeded
    ...
    [This error indicates that the message or operation failed due to
    memory or processing limitations on the target Endpoint.
    ],
    [`7006`
    ],
    [Permission denied
    ...
    [This error indicates that the source Endpoint does not have the
    ...
    [This error indicates that the message or operation failed because
    processing the message would put the target Endpoint in an invalid
    ...
    [This error indicates that the Path Name used was not understood by
    the target Endpoint.
    ],
    [`7009`
    ],
    ...
    [This error indicates that the Parameter failed to update for a
    general reason described in an err\_msg field.
    ],
    [`7010`
    ],
    ...
    [This error indicates that the requested Path Name associated with
    this ParamError or ParameterError did not match any instantiated
    Parameters.
    ],
    [`7011`
    ],
    ...
    [This error indicates that the received string can not be
    interpreted as a value of the correct type expected for the
    Parameter.
    ],
    [`7012`
    ],
    ...
    [This error indicates that the requested value was not within the
    acceptable values for the Parameter.
    ],
    [`7013`
    ],
    [Attempt to update non\-writeable parameter
    ],
    [Add, Set
    ],
    ...
    [This error indicates that the requested value would result in an
    invalid configuration based on other Parameter values.
    ],
    [`7015`
    ],
    [Operation error
    ...
    [This error indicates a general failure in the creation, update, or
    deletion of an Object that is described in an err\_msg field.
    ],
    [`7016`
    ],
    ...
    [This error indicates that the requested Path Name did not address
    an Object in the Agent’s Instantiated Data Model.
    ],
    [`7017`
    ],
    ...
    [This error indicates that the operation failed to create an
    ...
    [This error indicates that the requested Path Name is not a
    Multi\-Instance Object.
    ],
    [`7019`
    ],
    ...
    [This error indicates that the source Endpoint attempted to create
    an Object that is not defined as able to be created.
    ],
    [`7020`
    ],
    ...
    [This error indicates that the requested Object in a Set request
    failed to update.
    ],
    [`7021`
    ],
    ...
    [This error indicates that the request failed on this Object because
    one or more required Parameters failed to update. Details on the
    failed Parameters are included in an associated ParamError or
    ...
    [This error indicates that an command initiated in an Operate
    Request failed to complete for one or more reasons explained in the
    err\_msg field.
    ],
    [`7023`
    ],
    ...
    [This error indicates that an asynchronous command initiated in an
    Operate Request failed to complete because it was cancelled using
    ...
    [This error indicates that this Object Instance failed to be
    deleted.
    ],
    [`7025`
    ],
    ...
    [This error indicates that an Object already exists with the Unique
    Keys specified in an Add or Set Message.
    ],
    [`7026`
    ],
    [Invalid path
    ...
    [This error indicates that the Object, Parameter, or Command Path
    Name specified does not match any Objects, Parameters, or Commands
    in the Agent’s Supported Data Model
    ],
    [`7027`
    ],
    ...
    [This error indicates that an Operate Message failed due to invalid
    or unknown arguments specified in the command.
    ],
    [`7028`
    ],
    [Register failure
    ...
    [This error indicates that a path in a Register Request failed to be
    registered for one or more reasons explained in the err\_msg field.
    ],
    [`7029`
    ],
    [Already in use
    ...
    [This error indicates that a path in a Register Request failed to be
    registered, because it was registered by a different USP Agent
    ],
    [`7030`
    ],
    [Deregister failure
    ...
    [This error indicates that a path in a Deregister Request failed to
    be deregistered for one or more reasons explained in the err\_msg
    field.
    ],
    [`7031`
    ],
    ...
    [This error indicates that a path in a Deregister Request failed to
    be deregistered, because it was registered by a different USP Agent.
    ],
    [`7100-7199`
    ],
    ...
    [Data model defined error codes
    ],
    [\-
    ],
    [These errors are described in the data model.
    ],
    [`7800-7999`
    ],
...
ability to encrypt all MTPs using one of these two protocols, enable it
by default, and not implement the ability to disable it.
...
#strong[#[R\-SEC.0]<r-sec.0>]; \- Prior to processing a USP Message from
a Controller, the Agent MUST either:
...
- have a Trusted Broker’s certificate information and have a
  ...
  Trusted Broker
#strong[#[R\-SEC.0a]<r-sec.0a>]; \- Whenever a X.509 certificate is used
to authenticate a USP Endpoint, the certificate MUST contain a
...
#strong[#[R\-SEC.1]<r-sec.1>]; \- An Agent MUST confirm a Controller has
the necessary permissions to perform the requested actions in a Message
prior to performing that action.

#strong[#[R\-SEC.1a]<r-sec.1a>]; \- Agents SHOULD implement the
`Controller` Object with the `AssignedRole` Parameter (with at least
read\-only data model definition) and `InheritedRole` Parameter (if
allowed Roles can come from a trusted CA), so users can see what
Controllers have access to the Agent and their permissions. This will
help users identify rogue Controllers that may have gained access to the
Agent.
...
== 8.3 Trusted Certificate Authorities <sec:trusted-certificate-authorities>
...
#strong[#[R\-SEC.2]<r-sec.2>]; \- To confirm a certificate was signed by
a trusted CA, the Endpoint MUST contain information from one or more
...
#strong[#[R\-SEC.3]<r-sec.3>]; \- Where a CA is trusted to authenticate
...
#strong[#[R\-SEC.4]<r-sec.4>]; \- Where a CA is trusted to authorize a
...
#strong[#[R\-SEC.4a]<r-sec.4a>]; \- To confirm a certificate belongs to
a Trusted Broker, the Endpoint MUST contain information from one or more
...
#strong[#[R\-SEC.4b]<r-sec.4b>]; \- A Trusted Broker MUST confirm the
...
#strong[#[R\-SEC.4c]<r-sec.4c>]; \- A Trusted Broker MUST guarantee that
USP Records sent from clients contain the correct value for the
`from_id` field, tying it to the identity provided during the connection
...
#strong[#[R\-SEC.5]<r-sec.5>]; \- An Endpoint that generates a
self\-signed certificate MUST ensure that the certificate is suitable
...
#strong[#[R\-SEC.6]<r-sec.6>]; \- If an Agent allows Controllers to
provide self\-signed certificates, the Agent MUST assign such
Controllers an "untrusted" Role on first use.
...
#strong[#[R\-SEC.7]<r-sec.7>]; \- If an Agent allows Controllers to
provide self\-signed certificates, the Agent MUST have a means of
allowing an external entity to change the Role of each such Controller.
...
#strong[#[R\-SEC.8]<r-sec.8>]; \- An Endpoint that accepts self\-signed
...
#strong[#[R\-SEC.9]<r-sec.9>]; \- Controllers MUST authenticate Agents
...
#strong[#[R\-SEC.10]<r-sec.10>]; \- An Agent certificate MUST be
suitable for USP authentication as per the #link(<r-sec.0a>)[R\-SEC.0a]
requirement.

#strong[#[R\-SEC.10a]<r-sec.10a>]; \- The certificate `subjectAltName`
...
#strong[#[R\-SEC.11]<r-sec.11>]; \- If a single certificate is shared
among multiple Agents, those Agents MUST include a wild\-carded
`instance-id` in the Endpoint ID in the `subjectAltName` extension with
identical `authority-scheme` and `authority-id`.
...
#strong[#[R\-SEC.12]<r-sec.12>]; \- If a wildcard character is present
in the `instance-id` of an Endpoint ID in a certificate `subjectAltName`
...
#strong[#[R\-SEC.13]<r-sec.13>]; \- The Agent MAY have factory\-default
Challenge value(s) (strings or images) in its configuration.

#strong[#[R\-SEC.14]<r-sec.14>]; \- A factory\-default Challenge value
MUST be unique to the Agent. Re\-using the same passphrase among
multiple Agents is not permitted.

#strong[#[R\-SEC.15]<r-sec.15>]; \- A factory\-default Challenge value
MUST NOT be derivable from information the Agent communicates about
itself using any protocol at any layer.

#strong[#[R\-SEC.16]<r-sec.16>]; \- The Agent MUST limit the number of
tries for the Challenge value to be supplied successfully.

#strong[#[R\-SEC.17]<r-sec.17>]; \- The Agent SHOULD have policy to lock
out all use of Challenge values for some time, or indefinitely, if the
...
== 8.8 Analysis of Controller Certificates <sec:analysis-controller-certificates>
...
#strong[#[R\-SEC.19]<r-sec.19>]; \- An Agent capable of obtaining
absolute time SHOULD wait until it has accurate absolute time before
contacting a Controller. If an Agent for any reason is unable to obtain
absolute time, it can contact the Controller without waiting for
...
that involve absolute time, e.g.~not\-valid\-before and
not\-valid\-after certificate restrictions.

#strong[#[R\-SEC.20]<r-sec.20>]; \- An Agent that has obtained accurate
absolute time MUST validate those components of the Controller
certificate that involve absolute time.

#strong[#[R\-SEC.21]<r-sec.21>]; – An Agent MUST clear all cached
encryption session and Role authorization information when it reboots.

#strong[#[R\-SEC.22]<r-sec.22>]; \- When an Agent receives a USP Record,
the Agent MUST execute logic that achieves the same results as in the
...
#strong[#[R\-SEC.22a]<r-sec.22a>]; \- When an Agent receives a USP
Record, the Agent SHOULD execute logic that achieves the same results as
in the optional decision flow elements (identified with "OPT") from
#link(<fig:receive-record>)[Figure 19] and 
#link(<fig:no-secure-message-exchange>)[Figure 20].
...
#figure(
  ...
  #bbf-image("security/receive-record.png")<img:receiving-a-usp-record>
] <fig:receive-record>
...
#figure(
  ...
  #bbf-image("security/no-secure-message-exchange.png")<img:usp-record-without-usp-layer-secure-message-exchange>
] <fig:no-secure-message-exchange>

=== 8.8.2 Sending a USP Record <sec:sending-a-usp-record>

#strong[#[R\-SEC.23]<r-sec.23>]; \- When an Agent sends a USP Record,
the Agent MUST execute logic that achieves the same results as in the
mandatory decision flow elements (identified with "MUST") from
#link(<fig:send-record>)[Figure 21].

#strong[#[R\-SEC.23a]<r-sec.23a>]; \- When an Agent sends a USP Record,
...
#figure(
  ...
  #bbf-image("security/send-record.png")<img:sending-a-usp-record>
] <fig:send-record>

=== 8.8.3 Checking a Certificate <sec:checking-a-certificate>

#strong[#[R\-SEC.24]<r-sec.24>]; \- When an Agent analyzes a Controller
...
#strong[#[R\-SEC.24a]<r-sec.24a>]; \- When an Agent analyzes a
Controller certificate for authentication and determining permissions
(Role), the Agent SHOULD execute logic that achieves the same results as
in the optional decision flow elements (identified with "OPT") from
#link(<fig:check-cert>)[Figure 22] and 
#link(<fig:determine-role>)[Figure 23].

#strong[#[R\-SEC.25]<r-sec.25>]; \- When determining the inherited Role
to apply based on Roles associated with a trusted CA, only the first
matching CA in the chain will be used.

#figure(
  caption: [Figure 22 – Checking a Certificate
    ])[
  #bbf-image("security/check-cert.png")<img:checking-a-certificate>
] <fig:check-cert>
...
#figure(
  ...
  #bbf-image("security/determine-role.png")<img:determining-the-role>
] <fig:determine-role>
...
#strong[#[R\-SEC.26]<r-sec.26>]; \- If Trusted Brokers are supported,
and a Trusted Broker is encountered (from the optional "OPT" "Trusted
Broker cert?" decision diamonds in
...
#strong[#[R\-SEC.26a]<r-sec.26a>]; \- If Trusted Brokers are supported,
and a Trusted Broker is encountered (from the optional "OPT" "Trusted
Broker cert?" decision diamonds in
#link(<fig:no-secure-message-exchange>)[Figure 20, Figure 21]) the Agent
SHOULD execute logic that achieves the same results as in the optional
...
#figure(
  ...
  #bbf-image("security/broker-with-received-record.png")<img:trusted-broker-with-received-record>
...
#figure(
  ...
  #bbf-image("security/broker-with-sent-record.png")<img:trusted-broker-sending-a-record>
...
permissions are applied to the data model, be it the Supported Data
Model or the Instantiated Data Model, is described in the data model
description of these Parameters, Objects, and various Messages.
...
In this example, the device is configured with a single role that allows
a Controller to access the entire Data Model.
...
```
The next example builds upon the previous example where the device was
configured with a role that allows a Controller to access the entire
Data Model (`Permission.1`), but in this example there is an exception
added that prevents the Controller from accessing the
LocalAgent.ControllerTrust portion of the Data Model (`Permission.2`).
The reason this works is due to the Order parameter. Since the Order
parameter in the `Permission.2` instance (value of 2) is larger than
(higher priority) the Order parameter in the `Permission.1` instance
...
```
Conversely, that role could have been configured to allow access to the
permissions, but not allowed access to read, write, or receive
ValueChange notifications for the `Order` parameter.
...
```
Based on the above example device settings, if a Controller was assigned
the previously defined role and it attempted to perform an Add operation
on the Permission table where param\_settings included the `Order`
parameter as a required setting, the Add operation would fail because
the Controller does not have write permissions for the `Order`
parameter.
...
```
Now, if that same Controller attempted to perform that Add operation on
the Permission table but this time the param\_settings included the
`Order` parameter as a non\-required setting, the Add operation would
NOT fail due to the lack of write permissions for the `Order` parameter,
instead the value requested for the `Order` parameter would be ignored.
...
parameter values. When the `SecuredRoles` Parameter references an
instance of the Role table containing a Target with a Partial Path, then
any included Parameters that are not identified as `secured` will be
ignored. The Agent will have access to all `secured` parameters in that
path.

For example, if a Controller needed access to all `secured` Parameters
in the Data Model, then the `SecuredRoles` Parameter could be set to the
role that was defined in the previous example:
...
```
and `Device.LocalAgent.Controller.1.AssignedRole` \=
`"Device.LocalAgent.ControllerTrust.Role.1", "Device.LocalAgent. ControllerTrust.Role.3"`

When determining permissions for the `Device.LocalAgent.Controller.`
table, the Agent will first determine that for Role A `Permission.2`
takes precedence over `Permission.1` (55 > 3). For B, `Permission.5`
takes precedence over `Permission.1` (78 > 20). The union of A and B is
`r-xn` + `----` \= `r-xn`.

==== 8.9.2.4 Role Associated with a Credential or Challenge <sec:role-associated-with-a-credential-or-challenge>
...
#emph[Note: Even if it is not exposed through the data model, the Agent
...
=== 8.9.4 Controller Certificates and Certificate Validation <sec:controller-certificates-and-certificate-validation>
...
=== 8.9.7 Application of Modified Parameters <sec:application-of-modified-parameters>
...
#bbf-annex1[
= Annex A: Bulk Data Collection <sec:bulk-data-collection>]

#emph[Note: This Annex has been re\-written in the 1.2 version of the
...
- #strong[What data needs to be collected]; :: The set of
  Object\/Parameter Path Names that dictate the set of Parameters that
  will be included in each Bulk Data report. Anything included in this
  set should be considered a filter that is applied against the
  Instantiated Data Model at the time of report generation, which means
  that the generation of the report is not contingent upon the Path Name
  being present in the Instantiated Data Model at the time of report
  generation.
- #strong[How often does the data need to be collected]; :: The interval
  and time reference that dictates the frequency and cycle of report
...
- #strong[Where does the data need to be sent]; :: The destination of
  where the report needs to be delivered after it has been generated.
  This is specific to the Bulk Data collection mechanism being used:
  HTTP vs MQTT vs USP Event Notification.
- #strong[How does the data get sent]; :: The protocol used to send the
  data across the wire, the encoding of the data, and the format of the
  data. From a Protocol perspective, the HTTP Bulk Data collection
  mechanism utilizes either the HTTP or HTTPS protocols, the MQTT Bulk
...
#emph[Note: When a Bulk Data Collection Profile is either created or
...
#emph[Note: When a Bulk Data Collection Report is generated the Agent
...
or Parameters that are not present at that time.];

#bbf-annex2[
== A.2 HTTP Bulk Data Collection <sec:http-bulk-data-collection>]
...
#bbf-annex3[
=== A.2.1 Enabling HTTP\/HTTPS Bulk Data Communication <sec:enabling-httphttps-bulk-data-communication>]
...
#strong[#[R\-BULK.0]<r-bulk.0>]; \- In many scenarios Agents will
utilize "chunked" transfer encoding. As such, the Bulk Data Collector
MUST support the HTTP transfer\-coding value of "chunked".

#bbf-annex3[
=== A.2.2 Use of the URI Query Parameters <sec:use-of-the-uri-query-parameters>]
...
#strong[#[R\-BULK.1]<r-bulk.1>]; \- The Agent MUST transmit the device’s
...
```
#emph[Note: If the USP Endpoint ID should be transmitted together with
...
#bbf-annex3[
=== A.2.3 Use of HTTP Status Codes <sec:use-of-http-status-codes>]
...
#bbf-annex4[
==== A.2.3.1 HTTP Retry Mechanism <sec:http-retry-mechanism>]

#strong[#[R\-BULK.2]<r-bulk.2>]; \- When the Agent receives an
unsuccessful HTTP status code and the HTTP retry behavior is enabled,
the Agent MUST try to redeliver the data. The retry mechanism employed
for the transfer of bulk data using HTTP uses the same algorithm as is
used for #link(<sec:responses-and-retry>)[USP Notify retries].
...
#strong[#[R\-BULK.3]<r-bulk.3>]; \- Beginning with the tenth retry
attempt, the Agent MUST choose from the fixed maximum range. The Agent
will continue to retry a failed bulk data transfer until it is
successfully delivered or until the next reporting interval for the data
transfer becomes effective.

#strong[#[R\-BULK.4]<r-bulk.4>]; \- Once a bulk data transfer is
successfully delivered, the Agent MUST reset the retry count to zero for
the next reporting interval.

#strong[#[R\-BULK.5]<r-bulk.5>]; \- If a reboot of the Agent occurs, the
Agent MUST reset the retry count to zero for the next bulk data
transfer.

#bbf-annex4[
==== A.2.3.2 Processing of Content for Failed Report Transmissions <sec:processing-of-content-for-failed-report-transmissions>]
...
#bbf-annex3[
=== A.2.4 Use of TLS and TCP <sec:use-of-tls-and-tcp>]
...
#strong[#[R\-BULK.6]<r-bulk.6>]; \- Certain restrictions on the use of
TLS and TCP are defined as follows:
...
#emph[Note: If the Collection Server does not support TLS 1.2 or higher
with a cipher suite supported by the Agent, it may not be possible for
the Agent to establish a secure connection to the Collection Server.];
...
#bbf-annex3[
=== A.2.5 Bulk Data Encoding Requirements <sec:bulk-data-encoding-requirements>]
...
```
#strong[#[R\-BULK.7]<r-bulk.7>]; \- The "`media-type`" field and
"`charset`" Parameters MUST be present in the Content\-Type header.
...
```
#strong[#[R\-BULK.8]<r-bulk.8>]; \- The BBF\-Report\-Format custom
header MUST be present when transferring data to the Bulk Data Collector
from the Agent using HTTP\/HTTPS.

#bbf-annex2[
== A.3 MQTT Bulk Data Collection <sec:mqtt-bulk-data-collection>]
...
#bbf-annex3[
=== A.3.1 Enabling MQTT Bulk Data Communication <sec:enabling-mqtt-bulk-data-communication>]
...
#bbf-annex3[
=== A.3.2 Determining Successful Transmission <sec:determining-successful-transmission>]
...
#bbf-annex4[
==== A.3.2.1 Retrying Failed Transmissions <sec:retrying-failed-transmissions>]
...
#bbf-annex4[
==== A.3.2.2 Processing of Content for Failed Report Transmissions <sec:processing-of-content-for-failed-report-transmissions-1>]
...
#bbf-annex3[
=== A.3.3 Bulk Data Encoding Requirements <sec:bulk-data-encoding-requirements-1>]
...
```
#strong[#[R\-BULK.8a]<r-bulk.8a>]; \- The "`media-type`" field and
...
#bbf-annex2[
== A.4 USPEventNotif Bulk Data Collection <sec:uspeventnotif-bulk-data-collection>]
...
#bbf-annex3[
=== A.4.1 Enabling USPEventNotif Bulk Data Communication <sec:enabling-uspeventnotif-bulk-data-communication>]
...
#bbf-annex3[
=== A.4.2 Determining Successful Transmission <sec:determining-successful-transmission-1>]
...
#bbf-annex4[
==== A.4.2.1 Retrying Failed Transmissions <sec:retrying-failed-transmissions-1>]
...
#bbf-annex3[
=== A.4.3 Bulk Data Encoding Requirements <sec:bulk-data-encoding-requirements-2>]
...
#bbf-annex2[
== A.5 Using Wildcards to Reference Object Instances in the Report <sec:using-wildcards-to-reference-object-instances-in-the-report>]
...
#bbf-annex2[
== A.6 Using Alternative Names in the Report <sec:using-alternative-names-in-the-report>]
...
#bbf-annex4[
==== A.6.0.1 Using Object Instance Wildcards and Parameter Partial Paths with Alternative Names <sec:using-object-instance-wildcards-and-parameter-partial-paths-with-alternative-names>]
...
#strong[#[R\-BULK.9]<r-bulk.9>]; \- When the value of the Reference
...
#strong[#[R\-BULK.10]<r-bulk.10>]; \- When the value of the Reference
...
```
#strong[#[R\-BULK.10a]<r-bulk.10a>]; \- When the value of the Exclude
...
#bbf-annex2[
== A.7 Encoding of Bulk Data <sec:encoding-of-bulk-data>]
...
#bbf-annex4[
==== A.7.1.1 Defining the Report Layout of the Encoded Bulk Data <sec:defining-the-report-layout-of-the-encoded-bulk-data>]
...
```
#strong[#[R\-BULK.12]<r-bulk.12>]; \- The report format of
"`ParameterPerRow`" MUST format each Parameter using the
`ParameterName`, `ParameterValue` and `ParameterType` in that order.

#strong[#[R\-BULK.13]<r-bulk.13>]; \- The `ParameterType` MUST be the
Parameter’s base data type as described in TR\-106
#link(<ref-TR-106>)[\[2\]].

#bbf-annex4[
==== A.7.1.2 Layout of Content for Failed Report Transmissions <sec:layout-of-content-for-failed-report-transmissions>]

#emph[Note: This is only relevant for the HTTP variant of Bulk Data
Collection.];
...
#bbf-annex4[
==== A.7.1.3 CSV Encoded Report Examples <sec:csv-encoded-report-examples>]

#bbf-annex5[
===== A.7.1.3.1 CSV Encoded Reporting Using ParameterPerRow Report Format <sec:csv-encoded-reporting-using-parameterperrow-report-format>]
...
#bbf-annex5[
===== A.7.1.3.2 CSV Encoded Reporting Using ParameterPerColumn Report Format <sec:csv-encoded-reporting-using-parameterpercolumn-report-format>]
...
#bbf-annex3[
=== A.7.2 Encoding of JSON Bulk Data <sec:encoding-of-json-bulk-data>]
...
#bbf-annex4[
==== A.7.2.1 Defining the Report Layout of the Encoded Bulk Data <sec:defining-the-report-layout-of-the-encoded-bulk-data-1>]
...
"Report":[{...},{...}]
```
#emph[Note: Multiple instances of Report instances may exist when
previous reports have failed to be transmitted.];

#bbf-annex4[
==== A.7.2.2 Layout of Content for Failed Report Transmissions <sec:layout-of-content-for-failed-report-transmissions-1>]
...
#bbf-annex4[
==== A.7.2.3 Using the ObjectHierarchy Report Format <sec:using-the-objecthierarchy-report-format>]
...
```
#emph[Note: The translated JSON Object name does not contain the
trailing period "." of the leaf Object.];

#bbf-annex4[
==== A.7.2.4 Using the NameValuePair Report Format <sec:using-the-namevaluepair-report-format>]
...
#bbf-annex4[
==== A.7.2.5 Translating Data Types <sec:translating-data-types>]
...
#[
  ...
  #table(
    ...
    [String: base64 representation of the binary data.
    ],
    [boolean
    ],
    [Boolean
    ...
    [String represented as an ISO\-8601 timestamp.
    ],
    [hexBinary
    ],
...
#bbf-annex4[
==== A.7.2.6 JSON Encoded Report Example <sec:json-encoded-report-example>]
...
#bbf-appendix1[
= Appendix I: Software Module Management <sec:software-module-management>]
...
execution environments (such as Java, Linux, Linux Containers, OSGi,
Docker, crun, runC, etc.). Devices implementing these more robust
platforms are often capable of downloading new applications dynamically,
perhaps even from third\-party software providers. These new
applications might enhance the existing capabilities of the device or
enable the offering of new services.
...
#bbf-appendix2[
== I.1 Lifecycle Management <sec:lifecycle-management>]
...
#bbf-appendix2[
== I.2 Software Modules <sec:software-modules>]

A Software Module, managed through a USP\-based Software Module Manager,
is any software entity that will be installed on a device. This includes
modules that can be installed\/uninstalled and those that can be started
and stopped. All software on the device is considered a software module,
with the exception of the primary firmware, which plays a different
enough role that it is considered a separate entity.
...
Typical examples include Linux Containers, Docker, crun, runC. It is
also likely that these environments could be "layered," i.e., that there
could be one primary environment such as Linux on which one or more
child environments such as Linux Containers or crun are stacked. It is
the responsibility of a USP\-based Software Module Manager to manage the
layering and exposing the status of the EE through the Device:2 Data
Model #link(<ref-TR-181>)[\[3\]].

USP\-based Software Module Management does not attempt to address the
management of the primary firmware image, which is expected to be
managed via the device’s Firmware Image Objects defined in the Root data
model.
...
#bbf-appendix3[
=== I.2.1 Deployment Units <sec:deployment-units>]
...
#figure(
  ...
  #bbf-image("extensions/software-module-management/du_state.png")<img:deployment-unit-state-diagram>
...
- In the Installing state, the DU is in the process of being Installed
  ...
  attempting to Install (e.g., Docker for an OCI image).
- In the Installed state, the DU has been successfully downloaded and
  ...
  dependent.
#strong[#[R\-SMM.0]<r-smm.0>]; \- An installed DU MUST persist across
...
#strong[#[R\-SMM.1]<r-smm.1>]; \- The device MUST complete the requested
...
#bbf-appendix4[
==== I.2.1.1 UUID Generation <sec:uuid-generation>]
...
#bbf-appendix3[
=== I.2.2 Execution Units <sec:execution-units>]
...
#figure(
  ...
  #bbf-image("extensions/software-module-management/eu_state.png")<img:execution-unit-state-diagram>
...
#bbf-appendix2[
== I.3 Execution Environment Concepts <sec:execution-environment-concepts>]
...
#figure(
  ...
  #bbf-image("extensions/software-module-management/smm_concepts.png")<img:possible-multi-execution-environment-implementation>
] <fig:multi-exec-env>
...
#emph[Note that the above is merely an example; whether a device
...
#figure(
  ...
  #bbf-image("extensions/software-module-management/ee_state.png")<img:execution-environment-state-diagram>
...
#bbf-appendix3[
=== I.3.1 Managing Execution Environments <sec:managing-execution-environments>]
...
#bbf-appendix3[
=== I.3.2 Application Data Volumes <sec:application-data-volumes>]
...
#bbf-appendix3[
=== I.3.3 Signing Deployment Units <sec:signing-deployment-units>]
...
#bbf-appendix2[
== I.4 Fault Model <sec:fault-model>]
...
#bbf-appendix4[
==== I.4.1.1 Install Faults <sec:install-faults>]
...
#bbf-appendix4[
==== I.4.1.2 Update Faults <sec:update-faults>]
...
#bbf-appendix4[
==== I.4.1.3 Uninstall Faults <sec:uninstall-faults>]
...
#bbf-appendix1[
= Appendix II: Firmware Management of Devices with USP Agents <sec:firmware-management-of-devices-with-usp-agents>]
...
#bbf-appendix2[
== II.1 Getting the firmware image onto the device <sec:getting-the-firmware-image-onto-the-device>]
...
#bbf-appendix2[
== II.2 Using multiple firmware images <sec:using-multiple-firmware-images>]
...
#bbf-appendix3[
=== II.2.1 Switching firmware images <sec:switching-firmware-images>]
...
#bbf-appendix3[
=== II.2.2 Performing a delayed firmware upgrade <sec:performing-a-delayed-firmware-upgrade>]
...
#bbf-appendix3[
=== II.2.3 Recovering from a failed upgrade <sec:recovering-from-a-failed-upgrade>]
...
#bbf-appendix1[
= Appendix III: Device Proxy <sec:device-proxy>]
...
#bbf-appendix1[
= Appendix IV: Communications Proxying <sec:communications-proxying>]
...
- MTP Proxy: proxies USP messages at the MTP layer and below; does not
  ...
  sleeping devices \[#emph[Note: The MTP Proxy may choose to look at the
  USP Record to get information related to USP Endpoints, especially
  when proxying WebSocket MTP.];\]
- USP to Non\-USP Proxy: Proxies between USP and a non\-USP management
  or control protocol

#bbf-appendix2[
== IV.1 Proxying Building Block Functions <sec:proxying-building-block-functions>]
...
#figure(
  ...
  #table(
    ...
    [#emph[L3\/4 Translation Function];
    ],
    [Translates up to and including the IP and transport layer (e.g.,
    UDP, TCP) protocol headers, while leaving all higher layer protocol
    headers and payloads untouched.
    ],
    [#emph[MTP Translation Function];
    ],
    [Translates up to and including the Message Transfer Protocol (MTP)
    header, while leaving the USP Record untouched. Requires knowledge
    of how to bind USP to two or more MTPs.
    ],
    [#emph[USP to non\-USP Translation Function];
    ],
    [Translates all headers and the USP Record and USP Message into data
    model and headers of another management protocol. Requires proxy to
    have an Agent.
    ],
    [#emph[Caching Function];
    ],
    [Can hold on to USP Messages intended for Endpoints that are
    intermittently connected, until a time when that Endpoint is
    connected. The USP Message is not altered, so no Endpoint is
    required to be aware of the existence of this function.
    ],
    [#emph[Non\-USP Advertisement Function];
    ],
    [Responds to discovery queries using protocols other than USP (e.g.,
    DNS\-SD, DNS, DHCP) on behalf of Endpoints. May include a DNS
    Server. See Discovery section for formatting of various non\-USP
    discovery protocols in the context of USP.
    ],
    [#emph[Non\-USP Discovery Function];
    ],
    [Discovers Endpoints through discovery queries using protocols other
    than USP. See Discovery section for formatting of various non\-USP
    discovery protocols in the context of USP.
    ],
    [#emph[Agent USP Advertisement Function];
    ],
    [Maintains a USP data model table of discovered Agents. Requires an
    Agent.
    ]
  )]
...
#bbf-appendix2[
== IV.2 Discovery Proxy <sec:discovery-proxy>]
...
An #emph[Agent USP Advertisement Function]; would be needed to support
...
#bbf-appendix2[
== IV.3 Connectivity Proxy <sec:connectivity-proxy>]
...
Proxy, by adding the #emph[Caching Function]; to the MTP Proxy (see
Section 3).
...
#emph[L3\/4 Translation Function]; 1. Depending on whether the proxy is
on the same network as the proxied Endpoints: 1. #emph[Non\-USP
Discovery Function]; and\/or otherwise determined\/configured knowledge
of Agent(s) 1. #emph[Non\-USP Advertisement Function]; and\/or
#emph[Agent USP Advertisement Function];

The Connectivity Proxy can also include the #emph[Caching Function]; to
support Endpoints with intermittent connectivity.

#bbf-appendix2[
== IV.4 Message Transfer Protocol (MTP) Proxy <sec:message-transfer-protocol-mtp-proxy>]
...
+ Depending on whether it is on the same network as the proxied Agents
  ...
  + #emph[Non\-USP Discovery Function]; and\/or otherwise
    determined\/configured knowledge of Agent(s)
  + #emph[Non\-USP Advertisement Function]; and\/or #emph[Agent USP
    Advertisement Function];
The MTP Proxy can also include the #emph[Caching Function]; to support
Endpoints with intermittent connectivity.

#bbf-appendix3[
=== IV.4.1 MTP Header Translation Algorithms <sec:mtp-header-translation-algorithms>]
...
#figure(
  ...
  #table(
    ...
    [Maintain mapping of discovered\/configured info to advertised info
    ],
    [store discovered CoAP path\/url\/IP address\/port with "reply to"
    and\/or connectivity info for other MTP
    ],
    [generate a CoAP #emph[uri\-path]; for discovered info
    ],
    [],
    [Maintain mapping of received info
    ],
    [store received #emph[uri\-query reply\-to]; CoAP Parameter with
    "reply to" and\/or connectivity info of the sent message
    ],
    [store the supplied "reply to" and\/or connectivity info with a
    generated CoAP #emph[uri\-path];
    ],
    [],
    [Identify target USP Endpoint for a received message
    ],
    [possible source: received CoAP #emph[uri\-path];
    ],
    [put value from a maintained mapping in #emph[uri\-path]; and use IP
    address and port from mapping
    ],
    [WebSocket
    ],
    [Maintain mapping of discovered\/configured info to advertised info
    ],
    [store WebSocket connection info (and Endpoint ID, if socket is used
    for more than one Endpoint) with "reply to" and\/or connectivity
    info for other MTP
    ],
    [establish WebSocket connection or associate Endpoint with existing
    connection, for discovered info
    ],
    [],
    [Maintain mapping of received info
    ],
    ...
    [send over WebSocket connection associated with the proxied Endpoint
    ],
    [STOMP
    ],
    [Maintain mapping of discovered\/configured info to advertised info
    ],
    [store subscribed\-to STOMP destination with "reply to" and\/or
    connectivity info for other MTP
    ],
    [subscribe to STOMP destination for discovered info
    ],
    [],
    [Maintain mapping of received info
    ],
    [store #emph[reply\-to\-dest]; STOMP header (and associated STOMP
    connection) with "reply to" or socket info of the sent message
    ],
    [store the supplied "reply to" and\/or connectivity info with
    subscribed\-to STOMP destination and connection
    ],
    [],
    [Identify target USP Endpoint for a received message
    ],
    [possible source: received STOMP #emph[destination]; \
    possible source: to\_id in USP Record
    ],
    [put value from maintained mapping in STOMP destination header and
    use STOMP connection from that mapping
    ],
    [MQTT
    ],
    [Maintain mapping of discovered\/configured info to advertised info
    ],
    [store subscribed\-to Topic (Filter) with "reply to" and\/or
    connectivity info for other MTP
    ],
    [subscribe to MQTT Topic (Filter) for discovered info (if Topic
    Filter, know which specific Topic to use for "reply to" info)
    ],
    [],
    [Maintain mapping of received info
    ],
    [store Response Topic or other provided "reply to" info (and
    associated MQTT connection) with "reply to" or connectivity info of
    the sent message
    ],
    [store the supplied "reply to" and\/or connectivity info with a
    ...
    [put value from maintained mapping in MQTT `PUBLISH` Topic Name and
    use MQTT connection from that mapping
    ]
  )]
...
#figure(
  ...
  #bbf-image("extensions/proxying/MTP-proxy-example.png")<img:example-of-mtp-proxy-in-lan-with-wan-controller>
] <fig:example-of-mtp-proxy-in-lan-with-wan-controller>

#bbf-appendix3[
=== IV.4.2 CoAP \/ STOMP MTP Proxy Example Message Flow <sec:coap-stomp-mtp-proxy-example-message-flow>]
...
#figure(
  ...
  #bbf-image("extensions/proxying/CoAP-STOMP-MTP-proxy-example.png")<img:coap-stomp-mtp-proxy-example-flow>
] <fig:coap-stomp-mtp-proxy-example-flow>
#strong[Controller connects to the STOMP server];

#strong[A \/ B \/ C]; At any point prior to \#5 the USP Controller
Endpoint ctrl1 connects to STOMP and subscribes to destination A
...
#strong[\#1]; The USP Endpoint agent1 appears on the network. Proxy
receives advertisement and gets the USP Endpoint identifier "agent1" of
the Agent (retrieved from mDNS advertisement see
#link(<r-dis.8>)[R\-DIS.8]).

#strong[\#2]; Proxy sends a CONNECT frame to the STOMP server with
endpoint\-id header of "agent1".

#strong[\#3]; Proxy receives a subscribe\-dest header in the CONNECTED
frame identifying the STOMP destination it needs to subscribe to on
behalf of agent1.

#strong[\#4]; The Proxy sends a SUBSCRIBE frame to the STOMP server with
destination:Y and stores a mapping of USP Endpoint agent1 with
coaps:\/\/\<Agent IP>:\<port>\/agent1 to this STOMP connection with
destination Y.

#strong[\#5 \/ \#6]; USP Endpoint ctrl1 initiates USP message to agent.
Proxy creates a STOMP reply\-to\-dest:A (on this STOMP connection) to
coaps:\/\/\<Proxy IP>:\<port>\/destA mapping.

#strong[\#7\/ \#7.1]; Proxy takes USP Record from the STOMP frame and
sends it in a CoAP payload with CoAP URI coming from the step \#4
mapping of STOMP destination Y to coap:\/\/\<Agent IP>:\<port>\/agent1.
To secure the communication, the proxy and Agent establish a DTLS
session (exchange certificates) and the Agent determines whether the
proxy is a Trusted Broker.

#strong[\#8 \/ \#8.1]; USP Endpoint agent1 sends a USP Record in reply
to ctrl1 using CoAP, to coaps:\/\/\<Proxy IP>:\<port>\/destA.

#strong[\#9 \/ \#10]; Proxy takes USP Record from the CoAP payload and
sends it in a STOMP SEND frame using the mapping (created in steps \#5
\/ \#6) of coaps:\/\/\<Proxy IP>:\<port>\/destA to STOMP destination A
(and associated STOMP connection) created in steps \#5 \/ \#6 .

#strong[Agent sends Notify Message to Controller];
...
#strong[\#11]; The Agent sends mDNS query for
ctrl1.\_usp\-ctrl\-coap.\_udp.\_local.

#strong[\#12]; The Proxy response to the Agent includes TXT record with
path of coaps:\/\/\<Proxy IP>:\<port>\/ctrl1. This provides a URL for
the Agent to use to send a Notify Message to the Controller.

#strong[\#13 \/ \#13.1]; The Agent sends a Notify Message to Controller
at coaps:\/\/\<Proxy IP>:\<port>\/ctrl1.

#strong[\#14 \/ \#15]; Proxy takes the USP Record from the CoAP payload
and sends it in a STOMP SEND frame using the mapping (stored in \#5 \/
\#6 ) of coaps:\/\/\<Proxy IP>:\<port>\/destA to STOMP destination:A
(and associated STOMP connection).

#bbf-appendix2[
== IV.5 USP to Non\-USP Proxy <sec:usp-to-non-usp-proxy>]
...
In order to serve as a USP to non\-USP Proxy, the #emph[USP to non\-USP
Translation Function]; (from
#link(<tbl:proxy-building-block-functions>)[Table 1]) is needed.

#bbf-appendix1[
= Appendix V: IoT Data Model Theory of Operation <sec:iot-data-model-theory-of-operation>]

#bbf-appendix2[
== V.1 Introduction <sec:introduction-2>]
...
#bbf-appendix2[
== V.2 IoT data model overview <sec:iot-data-model-overview>]
...
#figure(
  ...
  #bbf-image("extensions/iot/./figure-73.png")<img:iot-data-model>
] <fig:iot-data-model>
...
#bbf-appendix3[
=== V.2.1 IoT Capability table <sec:iot-capability-table>]
...
#[
  ...
  #table(
    ...
    [Allows setting a binary value (e.g.~on or off)
    ],
    [`LevelControl`
    ],
    [Allows setting a continuous value in a predefined range
    ],
    [`EnumControl`
    ],
    [Allows setting a value from a predefined set of values
    ],
    [`BinarySensor`
    ],
    [Provides a binary reading value (true\/false)
    ],
    [`LevelSensor`
    ],
    [Provides a continuous reading value
    ],
    [`MultiLevelSensor`
    ],
    [Provides multiple reading values, which belong together
    ]
  )
]
...
#bbf-appendix3[
=== V.2.2 Node Object table <sec:node-object-table>]
...
#bbf-appendix2[
== V.3 Architecture mappings <sec:architecture-mappings>]

#bbf-appendix3[
=== V.3.1 Individual IoT devices <sec:individual-iot-devices>]
...
#figure(
  ...
  #bbf-image("extensions/iot/./figure-74.png")<img:iot-individual-device-models>
...
#bbf-appendix3[
=== V.3.2 Proxied IoT devices <sec:proxied-iot-devices>]
...
#figure(
  ...
  #bbf-image("extensions/iot/./figure-75.png")<img:iot-proxied-device-model>
...
#bbf-appendix2[
== V.4 IoT data model Object details <sec:iot-data-model-object-details>]

#bbf-appendix3[
=== V.4.1 Common capability Parameters <sec:common-capability-parameters>]

These Parameters have the same behavior for all capability Sub\-Objects,
where defined.

#bbf-appendix4[
==== V.4.1.1 Type definition <sec:type-definition>]

#strong[Applies to:]; All capability Sub\-Objects
...
#emph[Note: The `Type` enumeration in the data model can also, like all
Parameters, be extended using the rules defined in TR\-106
#link(<ref-TR-106>)[\[2\]].];

#bbf-appendix4[
==== V.4.1.2 Unit definition <sec:unit-definition>]

#strong[Applies to:]; LevelControl, LevelSensor, MultilevelSensor
...
#emph[Note: The `Unit` enumeration in the data model can also, like all
Parameters, be extended using the rules defined in TR\-106
#link(<ref-TR-106>)[\[2\]].];

#emph[Note: Imperial units are intentionally not modeled in favor of the
...
ones by using the well\-defined conversion routines.];

#bbf-appendix3[
=== V.4.2 Control Objects <sec:control-objects>]
...
#bbf-appendix4[
==== V.4.2.1 BinaryControl <sec:binarycontrol>]
...
#bbf-appendix4[
==== V.4.2.2 LevelControl <sec:levelcontrol>]
...
#bbf-appendix4[
==== V.4.2.3 EnumControl <sec:enumcontrol>]
...
#bbf-appendix3[
=== V.4.3 Sensor Objects <sec:sensor-objects>]
...
#bbf-appendix4[
==== V.4.3.1 Binary Sensor <sec:binary-sensor>]
...
#bbf-appendix5[
===== V.4.3.1.1 Simple binary state sensor <sec:simple-binary-state-sensor>]
...
#bbf-appendix5[
===== V.4.3.1.2 Threshold trigger sensor <sec:threshold-trigger-sensor>]
...
#figure(
  ...
  #bbf-image("extensions/iot/./figure-76.png")<img:iot-threshold-trigger-sensitivity>
...
#bbf-appendix5[
===== V.4.3.1.3 Trigger time control <sec:trigger-time-control>]
...
#figure(
  ...
  #bbf-image("extensions/iot/./figure-77.png")<img:iot-threshold-trigger-hold-time>
] <fig:iot-threshold-trigger-hold-time>
If the `HoldTime` Parameter is not implemented or is set to `0`, the
handling is disabled.

#bbf-appendix5[
===== V.4.3.1.4 Repeated trigger with grace period <sec:repeated-trigger-with-grace-period>]
...
#figure(
  ...
  #bbf-image("extensions/iot/./figure-78.png")<img:iot-threshold-trigger-rest-time>
] <fig:iot-threshold-trigger-rest-time>
If the `RestTime` Parameter is not implemented or is set to `0`, the
handling is disabled.

#bbf-appendix5[
===== V.4.3.1.5 Repeated trigger with minimum duration <sec:repeated-trigger-with-minimum-duration>]
...
#figure(
  ...
  #bbf-image("extensions/iot/./figure-79.png")<img:iot-threshold-trigger-minimum-duration>
] <fig:iot-threshold-trigger-minimum-duration>

#bbf-appendix4[
==== V.4.3.2 Level Sensor <sec:level-sensor>]
...
#bbf-appendix5[
===== V.4.3.2.1 Level reading <sec:level-reading>]
...
#bbf-appendix5[
===== V.4.3.2.2 Threshold trigger <sec:threshold-trigger>]
...
#[
  ...
  #table(
    ...
    [True means that the low level threshold is reached or undershot.
    ],
    [`LowLevelThreshold`
    ],
    [decimal
    ...
    [True means that the high level threshold is reached or exceeded.
    ],
    [`HighLevelThreshold`
    ],
    [decimal
...
```
#emph[Note: For more complex scenarios, like having a grace period, the
binary sensor Object can be used instead of the LowLevel or HighLevel
Threshold Parameters.];

#bbf-appendix5[
===== V.4.3.2.3 Multi Level Sensor <sec:multi-level-sensor>]
...
#bbf-appendix4[
==== V.4.3.3 Enum Sensor <sec:enum-sensor>]
...
#bbf-appendix2[
== V.5 Examples <sec:examples>]

This chapter gives several examples how to model IoT Devices.

#bbf-appendix3[
=== V.5.1 Example: A\/C Thermostat <sec:example-ac-thermostat>]
...
#bbf-appendix3[
=== V.5.2 Example: Light with a dimmer and switch <sec:example-light-with-a-dimmer-and-switch>]
...
#bbf-appendix3[
=== V.5.3 Example: Fan <sec:example-fan>]
...
#bbf-appendix3[
=== V.5.4 Example: Multi\-Sensor strip with a common battery. <example-multi-sensor-strip-with-a-common-battery>]
...
#bbf-appendix3[
=== V.5.5 Example: Ceiling Fan with integrated light <sec:example-ceiling-fan-with-integrated-light>]
...
#bbf-appendix3[
=== V.5.6 Example: Power strip <sec:example-power-strip>]
...
#bbf-appendix3[
=== V.5.7 Example: Battery powered radiator thermostat <sec:example-battery-powered-radiator-thermostat>]
...
#emph[Note: All temperature settings are modeled as "EnumControl" to
...
#bbf-appendix1[
= Appendix VI: Software Modularization and USP\-Enabled Applications Theory of Operation <sec:software-modularization-theory-of-operations>]
...
#bbf-appendix2[
== VI.1 Background <sec:background>]
...
#bbf-appendix2[
== VI.2 Basic Solution Concepts <sec:basic-solution-concepts>]
...
- USP Broker:
  - #strong[An entity that is responsible for exposing a consolidated
    set of Service Elements for the device to external USP Controllers.
    This includes any data model elements exposed by the USP Agent
    contained within the USP Broker as well as any data model elements
    exposed by USP Services that have connected to the USP Broker.
    Furthermore, the USP Broker serves as an intermediary for USP
    Services looking to interact with data model elements that are
    maintained by other portions of the device (the USP Broker or other
    USP Services).];
...
- USP Service:
  - #strong[An entity that is responsible for implementing a portion of
...
- UNIX Domain Socket MTP:
  - #strong[An internal MTP for communications within the device via
    UNIX Domain Sockets.];
  ...
  - While authentication is not required in the case of trusted
    applications being installed, there is an optional TLS\-based
    authentication mechanism that is defined within the UNIX domain
    socket (UDS) MTP section.

#bbf-appendix3[
=== VI.2.1 Usage of the Register Operation <sec:usage-of-the-register-operation>]

The following rules detail the usage of the Register Operation by a USP
Service as well as how that usage impacts other USP Services.

+ A USP Service cannot register something that is already registered by
  another USP Service. For example: If Service 1 registers
  #strong[Device.WiFi.DataElements]; first, then Service 2 attempts to
  register #strong[Device.WiFi.DataElements]; \- that results in a
  failure.
+ A USP Service cannot register something that has a sub\-object that is
  already registered by another USP Service. For example: If Service 1
  registers #strong[Device.WiFi.DataElements]; first, then Service 2
  attempts to register #strong[Device.WiFi]; \- that results in a
  failure; Service 2 should only register what it needs to instead of
  attempting to register all of WiFi.
+ A USP Service cannot register something that is a sub\-object of
  something that is already registered by another USP Service. For
  example: If Service 1 registers #strong[Device.WiFi]; first, then
  Service 2 attempts to register #strong[Device.WiFi.DataElements]; \-
  that results in a failure; Service 1 should only register what it
  needs to instead of attempting to register all of WiFi.
A USP Service is expected to only register the portion of the data model
that it is responsible for implementing, but if that USP Service expects
no overlap then it could register a sub\-Object of the root data model
object. For example, if Service 1 intends to implement all of
#strong[Device.WiFi]; without any overlaps, then it would register
#strong[Device.WiFi];. However, if Service 1 and Service 2 expect an
overlap at the #strong[Device.WiFi]; level (due to not implementing the
full breadth of the Wi\-Fi Object), then Service 1 would register
#strong[Device.WiFi.DataElements]; and Service 2 would register
#strong[Device.WiFi.RadioNumberOfEntries];, #strong[Device.WiFi.Radio];.

#bbf-appendix2[
== VI.3 USP Service Use Cases <sec:usp-service-use-cases>]

The following 3 use cases represent 3 unique types of USP Services.

+ Data Model Provider Application
  + #strong[Description:]; USP Service that exposes a data model
  + #strong[Example:]; A Software Module that implements a Speed Test
  + #strong[Components:];
    + USP Agent (data model provider)
+ Integrated Data Model Application
  + #strong[Description:]; USP Service that both exposes a data model
    and needs to interact with dependent portions of the data model
    being exposed by other entities
  + #strong[Example:]; A Software Module that implements a Network
    Topology View
  + #strong[Components:];
    + USP Agent (data model provider)
    + USP Controller (data model consumer)
+ Cloud Application
  + #strong[Description:]; USP Service that resides in the cloud
  + #strong[Example:]; A Software Module that implements a cloud\-based
    Wi\-Fi mesh controller
  + #strong[Components:]; Could be any Agent\/Controller combination as
...
#figure(
  ...
  #bbf-image("extensions/device-modularization/./use-cases.png")<img:software-modularization-use-cases>
] <fig:software-modularization-use-cases>

#bbf-appendix2[
== VI.4 USP Broker Responsibilities <sec:usp-broker-responsibilities>]
...
#bbf-appendix2[
== VI.5 Data Model Implications for USP Brokers and USP Services <sec:data-model-implications-for-usp-brokers-and-usp-services>]

#bbf-appendix3[
=== VI.5.1 UNIX Domain Socket Data Model Table and the UDS MTP Objects <sec:unix-domain-socket-data-model-table-and-the-uds-mtp-objects>]
...
#bbf-appendix3[
=== VI.5.2 USPService Data Model Table <sec:uspservice-data-model-table>]
...
- #strong[EndpointID:]; the Endpoint ID of the USP Agent within the USP
  Service
- #strong[DataModelPaths:]; a list of data model paths that have been
  registered by the USP Service
- #strong[DeploymentUnitRef:]; a reference to the Software Module
  Deployment Unit, if applicable
- #strong[HasController:]; a flag that indicates whether or not the USP
  Service has an embedded USP Controller (NOTE: this can be determined
  when the USP Service’s USP Controller connects to the USP Broker’s USP
  Agent if it is using the same Endpoint ID as the USP Service’s USP
  Agent)
When a USP Service disconnects then the associated USPService table
instance is removed.

#bbf-appendix3[
=== VI.5.3 Example Data Models for a USP Broker and USP Services <sec:example-data-models-for-a-usp-broker-and-usp-services>]
...
#bbf-appendix2[
== VI.6 Startup and Shutdown Procedures <sec:startup-and-shutdown-procedures>]

#bbf-appendix3[
=== VI.6.1 Device Boot Procedures <sec:device-boot-procedures>]
...
#bbf-appendix3[
=== VI.6.2 USP Service Startup Procedures <sec:usp-service-startup-procedures>]

#emph[Use Case 1 \- Data Model Provider Application:];
...
- The Agent within the USP Service initiates the UNIX Domain Socket
  ...
  internal path
  - Once the UNIX Domain Socket is connected, the USP Agent and USP
    Controller can perform TLS handshaking, if desired
  - Once the UNIX Domain Socket is connected (and TLS handshaking is
    complete, if desired), the USP Service’s Agent will initiate the
    UNIX Domain Socket MTP Handshake mechanism
...
#emph[Use Case 2 \- Integrated Data Model Application:];
...
- The Controller within the USP Service initiates the UNIX Domain Socket
  ...
  - Once the USP Service identifies that it is connected to the USP
    ...
    to interact with
#emph[Use Case 3 \- Cloud Application:];

#strong[Note:]; One of the key tenets of USP was that multiple MTPs were
defined not for general preferences but because each of them serves a
different kind of use case. ~So when we define a new use case (like this
one), it is certainly conceivable that some MTPs might not be
...
- The~USP Broker establishes a connection to the STOMP\/MQTT Broker
  ...
  - #strong[Note:]; The USP Controller will need to configure the USP
    Broker to communicate with the Cloud USP Service by setting up the
    associated MTP connection details.
  - The Agent within the USP Broker sends a STOMP\/MQTT Connect Record
    to the Controller of the Cloud USP Service
    - #strong[Note:]; This looks just like any other external USP
      Controller that is configured within the USP Broker’s Agent.
...
#bbf-appendix3[
=== VI.6.3 USP Service Shutdown Procedures <sec:usp-service-shutdown-procedures>]
...
#bbf-appendix2[
== VI.7 USP Services and Software Modules <sec:usp-services-and-software-modules>]
...
#bbf-appendix3[
=== VI.7.1 Installing a Software Module <sec:installing-a-software-module>]
...
#bbf-appendix3[
=== VI.7.2 Updating a~Software Module <sec:updating-a-software-module>]
...
#bbf-appendix3[
=== VI.7.3 Deleting a Software Module <sec:deleting-a-software-module>]
