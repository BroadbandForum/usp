...
**R-COAP.3** - When creating a DNS-SD record (see [Discovery](/specification/discovery/)), an Endpoint MUST set the DNS-SD "path" attribute equal to the value of the CoAP server identifier (uri-path).
...
USP Records are transferred using the CoAP resource that represents the receiving USP Endpoint using the CoAP POST method as defined in [RFC 7252][9].
